...
export class StorePathsService {
  ...
  private pathsSubject = new BehaviorSubject<Array<string> | null>(this.getStoredPaths());
  public paths$: Observable<Array<string> | null> = this.pathsSubject.asObservable();
  ...
  private setPaths(paths: Array<string>): void {
    this.encodingDataService.saveData(this.STORAGE_KEY, JSON.stringify(paths));
