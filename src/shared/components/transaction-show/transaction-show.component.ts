...
import {
    COMMANDE_SIM,
    LIGNE_CREDIT,
} from 'src/presentation/pages/provisionning/provisionning-routing.module';
...
export class TransactionShowComponent implements OnInit {
    ...
    public operationIdentification: string =
            ...
            .subscribe({
                next: (response) => {
                    ...
                    if (
                        ...
                        this.detailTransaction?.operation ===
                            OperationTransaction.INTEGRATION
                    ) {
    ...
    OnFeebackTransaction() {
        if (
            ...
            this.router.url === `/${SUPERVISION_OPERATIONS}/${DEMANDE_ROUTE}`
        ) {
            return this.GetAllDemandes();
        } else if (
            this.router.url === `/${OPERATION_PROVISIONNING}/${COMMANDE_SIM}`
        ) {
            return this.GetAllAchats();
        } else if (
            this.router.url === `/${OPERATION_PROVISIONNING}/${LIGNE_CREDIT}`
        ) {
            return this.GetAllLigneCredits();
        } else if (
            this.router.url === `/${SUPERVISION_OPERATIONS}/${LIGNE_CREDIT}`
        ) {
            return this.GetAllLigneCredits();
        }
    }
    ...
    public GetAllTransactions() {
        this.patrimoineService.GetAllTransactions({}, 1).subscribe({
            next: (response) => {
                this.listTraitemants = response['data']['data'].map((data) => {
                    if (data?.statut === StatutTransaction.TARITER) {
                        return { ...data, current_date: data?.date_traitement };
    ...
    public GetAllAchats() {
        this.provisionningService.GetAllAchats({}, 1).subscribe({
            next: (response) => {
    ...
    public GetAllLigneCredits() {
        this.provisionningService.GetAllLigneCredits({}, 1).subscribe({
            next: (response) => {
        ...
        if (propiete === 'montant_formule') {
            const formatFCA =
                new Intl.NumberFormat('fr-FR').format(
                    detailTransaction?.['montant_formule']
                ) + ' CFA';
            return formatFCA;
        }
    }
    ...
    OnShowFormuleForm() {
        ...
        this.formuleForm
            .get('montant_formule')
            .patchValue(this.detailTransaction?.montant_formule);
        this.formuleForm
            .get('statut_contrat')
            .patchValue(this.detailTransaction?.statut_contrat);
        this.formuleForm
    ...
    OnInitIntegrationForm() {
        this.integrationForm = this.fb.group({
            niveau_un_uuid: [
                this.createFormControl(null, Validators.required, true),
            ],
            niveau_deux_uuid: [
                this.createFormControl(null, Validators.required, true),
            ],
    ...
    OnShowIntegrationForm() {
        this.integrationForm.patchValue({
            ...
            adresse_geographique: [
                this.detailTransaction?.adresse_geographique,
            ],
            ...
            montant_formule: [
                this.pipeValue(this.detailTransaction, 'montant_formule'),
            ],
            ...
            integration_accepte_comment: [
                this.detailTransaction?.integration_accepte_comment,
            ],
        });
    }
    ...
    public IscurrentDate(): string {
        if (this.transaction?.statut === StatutTransaction.TARITER) {
            ...
            this.transaction?.traitement === StatutTransaction.SOUMIS &&
            this.transaction?.traitement === TraitementTransaction.ACQUITER
        ) {
