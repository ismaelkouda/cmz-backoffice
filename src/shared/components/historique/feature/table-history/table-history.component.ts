...
import { TableConfig, TableExportExcelFileService } from "../../../../services/table-export-excel-file.service";
...
export class TableHistoryComponent {
    ...
    constructor(private toastService: ToastrService, private clipboardService: ClipboardService, private ngbModal: NgbModal,
        private historyApiService: HistoryApiService, private tableExportExcelFileService: TableExportExcelFileService,
    ...
    public onExportExcel(): void {
        this.listHistory$.subscribe(data => {
            if (data) {
                this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "list_histories");
            }
        });
    }
    ...
    public handleAction(params: Action): void {
        switch (params.view) {
            case 'modal': if (params.action === 'view-history') { this.handleDetailsHistory(params.data) } break;
        }
    }
