...
export class HistoriqueComponent implements OnInit {
  ...
  constructor(private loadingBar: LoadingBarService, private toastrService: ToastrService,
  ...
  ngOnInit(): void {
    ...
    this.historyApiService.fetchHistory({ idModel: this.idModel, typeModel: this.typeModel, ...this.formFilter.value });
  ...
  async getAllUsers(dataToSend: Object = {}): Promise<any> {
    ...
    this.listUsers = this.response?.data.map((user: any) => {
      return { ...user, fullName: `[${user.matricule}] ${user.nom} ${user.prenoms}` };
    });
  }
  async pageCallback(nbrPage: string = '1') {
    ...
    const dataToSend = { idModel: this.idModel, typeModel: this.typeModel, ...this.formFilter.value };
    this.response = await handle(() => this.settingService.getHistoriques(dataToSend, nbrPage), this.toastrService, this.loadingBar);
