...
export class MyAccountComponent implements OnInit {
  ...
  async handleUpdatePassword(dataToSend: Object = this.passwordForm.value): Promise<void> {
    this.submitted = true;
    if (this.passwordForm.get('new_password').value !== this.passwordForm.get('new_password_confirmation').value) {
      this.passwordForm.get('new_password')?.setErrors({ invalidPassword: true });
      this.passwordForm.get('new_password_confirmation')?.setErrors({ invalidPassword: true });
    }
    if (this.passwordForm.valid) {
      const response: any = await handle(() => this.settingService.HandleUpdatePassword(dataToSend), this.toastrService, this.loadingBarService);
