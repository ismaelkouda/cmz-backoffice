...
import { TABLE_FORM_MASS_DEMAND, TABLE_FORM_SIMPLE_DEMAND } from './data-access/contantes/form-demand-table';
...
export class FormDemandComponent implements OnInit {
    ...
    public initFormDemand(): void {
        this.formDemand = this.fb.group<IFormDemandValues>({
            source: new FormControl<string>("stock orange", { nonNullable: true }),
            operation: new FormControl<string>(this.urlParamTypeDemand, { nonNullable: true }),
            niveau_un_uuid: new FormControl<string>('', { nonNullable: true }),
            niveau_deux_uuid: new FormControl<string>('', { nonNullable: true }),
            niveau_trois_uuid: new FormControl<string>('', { nonNullable: true }),
            usage_id: new FormControl<string | null>(null, { nonNullable: true }),
            nb_demandes: new FormControl<number>(0, {
                nonNullable: true,
                validators: [Validators.required, Validators.min(1), Validators.pattern(/^\d+(\.\d{1,2})?$/)],
            }),
            point_emplacement: new FormControl<string>('', { nonNullable: true }),
            adresse_geographique: new FormControl<string>('', { nonNullable: true }),
            adresse_email: new FormControl<string>('', { validators: Validators.email, nonNullable: true }),
            longitude: new FormControl<string>('', { nonNullable: true }),
            latitude: new FormControl<string>('', { nonNullable: true }),
            formule_uuid: new FormControl<string | null>(null, { validators: Validators.required, nonNullable: true }),
        ...
        this.formDemand?.get('niveau_un_uuid')?.valueChanges.subscribe(
            this.fetchSecondLevel.bind(this)
        );

        // Récupération des contrôles
        const pointEmplacementControl = this.formDemand.get("point_emplacement");
        ...
        nbDemandControl?.valueChanges.pipe(distinctUntilChanged()).subscribe(value => {
            gererMontantDemandes(value);
        });

        sourceControl?.valueChanges.pipe(distinctUntilChanged()).subscribe(value => {
    ...
    private getTypeDemand(urlParamTypeDemand: T_OPERATION): T_OPERATION {
        ...
        } else if (urlParamTypeDemand.includes(OperationTransaction.SIM_BLANCHE)) {
    ...
    expandAll() {
        this.expandedRows = this.listWhiteSimCardAvailable.reduce((acc, p) => (acc[p.id] = true) && acc, {});
    }
    ...
    saveRowEdit(WhiteSimCard: any): void {
        ...
        const updatedData = {
            ...
            quantite: WhiteSimCard.selected_nb_restants ?? originalWhiteSimCard.selected_nb_restants,
        };
        ...
        if (!existsInEditRow) {
            ...
            const EDIT_SUCCESSFULLY_ADDED = this.translate.instant('EDIT_SUCCESSFULLY_ADDED');
    ...
    cancelRowEdit(customer: any, rowIndex: number): void {
        ...
        this.listWhiteSimCardAvailable[rowIndex] = this.clonedWhiteSimCard[customer.id];
