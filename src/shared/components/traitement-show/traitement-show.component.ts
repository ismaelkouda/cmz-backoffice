...
export class TraitementShowComponent implements OnInit {
    ...
    public justificatifError: string =
        'AUCUN JUSTIFICATIF POUR CETTE TRANSACTION';
    ...
    public TextInfosVolume: string =
        "Orange CI fournira le volume, à l'issue de l'operation une facture instantannée sera produite";
    ...
    public GetDetailTransaction() {
        ...
        this.supervisionOperationService
            .GetDetailTransaction({
                ...
                ...(this.router.url !== `/${PATRIMOINE}/${''}`
                    ? { model_id: this.transaction?.model_id }
                    : { model_id: this.transaction?.demande_id }),
            ...
            .subscribe({
                next: (response) => {
                    ...
                    if (
                        this.detailTransaction?.operation ===
                        OperationTransaction.PROVISIONNING
                    ) {
                        ...
                        this.detailTransaction?.operation ===
                        OperationTransaction.CHANGEMENT_FORMULE
                    ) {
                        this.GetAllFormules();
                        ...
                        this.detailTransaction?.operation ===
                        OperationTransaction.ACHAT_SERVICE
                    ) {
                    ...
                    if (this.ShowAcceptedForm()) {
                        this.activationForm
                            .get('activation_accepte_comment')
                            .disable();
                        this.suspensionForm
                            .get('suspension_accepte_comment')
                            .disable();
                        this.formuleForm
                            .get('chg_formule_accepte_comment')
                            .disable();
                        this.resiliationForm
                            .get('resiliation_accepte_comment')
                            .disable();
                        this.swapForm.get('swap_accepte_comment').disable();
                        this.volumeForm
                            .get('volume_data_accepte_comment')
                            .disable();
                        this.achatForm
                            .get('commmande_produit_accepte_comment')
                            .disable();
                        this.ligneForm
                            .get('provisionning_accepte_comment')
                            .disable();
                    }
    ...
    IsContencieutRouter(): boolean {
        if (
            this.router.url ===
            `/${SUPERVISION_OPERATIONS}/${CONTENCIEUX_ROUTE}`
        ) {
            return true;
        }
    }
    public GetCurrentMessage(operation): string {
        switch (operation) {
            case OperationTransaction.ACTIVATION: {
                return this.activationForm.get('activation_accepte_comment')
                    .value;
            }
            ...
            case OperationTransaction.RESILIATION: {
                return this.resiliationForm.get('resiliation_accepte_comment')
                    .value;
            }
            case OperationTransaction.SUSPENSION: {
                return this.suspensionForm.get('suspension_accepte_comment')
                    .value;
            }
            case OperationTransaction.CHANGEMENT_FORMULE: {
                return this.formuleForm.get('chg_formule_accepte_comment')
                    .value;
            }
            ...
            case OperationTransaction.ACHAT_SERVICE: {
                return this.achatForm.get('commmande_produit_accepte_comment')
                    .value;
            }
            case OperationTransaction.PROVISIONNING: {
                return this.ligneForm.get('provisionning_accepte_comment')
                    .value;
            }
            default:
    ...
    public GetAllTraitement() {
        this.supervisionOperationService.GetAllTransactions({}, 1).subscribe({
            next: (response) => {
    ...
    public GetAllContencieux() {
        this.supervisionOperationService.GetAllContencieux({}).subscribe({
            next: (response) => {
                this.listTraitemants = response['data']['data'].map((data) => {
                    if (data?.statut === StatutTransaction.TARITER) {
                        return { ...data, current_date: data?.date_traitement };
    ...
    OnShowLigneForm() {
        this.ligneForm
            .get('montant')
            .patchValue(this.detailTransaction?.montant);
        this.ligneForm
            .get('type_justificatif')
            .patchValue(this.detailTransaction?.type_justificatif);
        this.ligneForm
            .get('description')
            .patchValue(this.detailTransaction?.description);
        this.ligneForm
            .get('provisionning_accepte')
            .patchValue(this.detailTransaction?.rapport?.provisionning_accepte);
        this.ligneForm
            .get('provisionning_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.provisionning_accepte_comment
            );
    }
    downloadFile() {
        if (!this.detailTransaction?.justificatif) {
            this.toastrService.warning(
                'Pas de justificatif pour cette operation'
            );
        } else {
    ...
    public downloadRecuPaiement() {
        if (!this.detailTransaction?.recu_paiement) {
            this.toastrService.warning(
                'Pas de recu de paiement pour cette operation'
            );
        } else {
    ...
    OnShowVolumeForm() {
        ...
        this.volumeForm
            .get('msisdn')
            .patchValue(this.detailTransaction?.msisdn);
        this.volumeForm
            .get('statut_contrat')
            .patchValue(this.detailTransaction?.statut_contrat);
        this.volumeForm
            .get('point_emplacement')
            .patchValue(this.detailTransaction?.point_emplacement);
        this.volumeForm
            .get('bac_a_pioche')
            .patchValue(this.detailTransaction?.bac_a_pioche);
        this.volumeForm
            .get('volume')
            .patchValue(this.detailTransaction?.volume);
        this.volumeForm
            .get('description')
            .patchValue(this.detailTransaction?.description);
        this.volumeForm
            .get('volume_data_accepte')
            .patchValue(this.detailTransaction?.rapport?.volume_data_accepte);
        this.volumeForm
            .get('volume_data_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.volume_data_accepte_comment
            );
    ...
    OnShowSwapForm() {
        ...
        this.swapForm
            .get('swap_accepte')
            .patchValue(this.detailTransaction?.rapport?.swap_accepte);
        this.swapForm
            .get('swap_accepte_comment')
            .patchValue(this.detailTransaction?.rapport?.swap_accepte_comment);
    ...
    OnShowResiliationForm() {
        this.resiliationForm
            .get('imsi')
            .patchValue(this.detailTransaction?.imsi);
        this.resiliationForm
            .get('msisdn')
            .patchValue(this.detailTransaction?.msisdn);
        this.resiliationForm
        ...
        this.resiliationForm
            ...
            .patchValue(this.detailTransaction?.description);
        this.resiliationForm
            .get('resiliation_accepte')
            .patchValue(this.detailTransaction?.rapport?.resiliation_accepte);
        this.resiliationForm
            .get('resiliation_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.resiliation_accepte_comment
            );
    ...
    OnShowSuspensionForm() {
        ...
        this.suspensionForm
            ...
            .patchValue(this.detailTransaction?.description);
        this.suspensionForm
            .get('suspension_accepte')
            .patchValue(this.detailTransaction?.rapport?.suspension_accepte);
        this.suspensionForm
            .get('suspension_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.suspension_accepte_comment
            );
    ...
    OnShowFormuleForm() {
        ...
        this.formuleForm
            .get('formule')
            .patchValue(this.detailTransaction?.formule);
        this.formuleForm
            .get('statut_contrat')
            .patchValue(this.detailTransaction?.statut_contrat);
        this.formuleForm
        ...
        this.formuleForm
            .get('formule_uuid')
            .patchValue(this.detailTransaction?.formule_uuid);
        this.formuleForm
            .get('description')
            .patchValue(this.detailTransaction?.description);
        this.formuleForm
            .get('chg_formule_accepte')
            .patchValue(this.detailTransaction?.rapport?.chg_formule_accepte);
        this.formuleForm
            .get('chg_formule_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.chg_formule_accepte_comment
            );
    ...
    OnShowActivationForm() {
        this.activationForm
            .get('type_personne')
            .patchValue(this.detailTransaction?.type_personne);
        this.activationForm.get('nom').patchValue(this.detailTransaction?.nom);
        this.activationForm
            .get('prenoms')
            .patchValue(this.detailTransaction?.prenoms);
        this.activationForm
            .get('nature_piece')
            .patchValue(this.detailTransaction?.nature_piece);
        this.activationForm
            .get('numero_piece')
            .patchValue(this.detailTransaction?.numero_piece);
        ...
        this.activationForm
            .get('niveau_un_uuid')
            .patchValue(this.detailTransaction?.niveau_un_uuid);
        this.activationForm
            .get('niveau_deux_uuid')
            .patchValue(this.detailTransaction?.niveau_deux_uuid);
        this.activationForm
            .get('niveau_trois_uuid')
            .patchValue(this.detailTransaction?.niveau_trois_uuid);
        this.activationForm
            .get('imsi')
            .patchValue(this.detailTransaction?.imsi);
        this.activationForm
        ...
        this.activationForm
            .get('iccid')
            .patchValue(this.detailTransaction?.iccid);
        this.activationForm.get('apn').patchValue(this.detailTransaction?.apn);
        this.activationForm
            .get('adresse_ip')
            .patchValue(this.detailTransaction?.adresse_ip);
        this.activationForm
            .get('formule')
            .patchValue(this.detailTransaction?.formule);
        this.activationForm
        ...
        this.activationForm
            .get('code_pin')
            .patchValue(this.detailTransaction?.code_pin);
        this.activationForm
            .get('usage_id')
            .patchValue(this.detailTransaction?.usage_id);
        this.activationForm
            .get('point_emplacement')
            .patchValue(this.detailTransaction?.point_emplacement);
        this.activationForm
            .get('adresse_email')
            .patchValue(this.detailTransaction?.adresse_email);
        this.activationForm
            .get('adresse_geographique')
            .patchValue(this.detailTransaction?.adresse_geographique);
        this.activationForm
            .get('latitude')
            .patchValue(this.detailTransaction?.latitude);
        this.activationForm
            .get('longitude')
            .patchValue(this.detailTransaction?.longitude);
        this.activationForm
            .get('description')
            .patchValue(this.detailTransaction?.description);
        this.activationForm
            .get('activation_accepte')
            .patchValue(this.detailTransaction?.rapport?.activation_accepte);
        this.activationForm
            .get('activation_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport?.activation_accepte_comment
            );
    ...
    OnShowAchatForm() {
        this.achatForm
            .get('commmande_produit_accepte')
            .patchValue(
                this.detailTransaction?.rapport?.commmande_produit_accepte
            );
        this.achatForm
            .get('commmande_produit_accepte_comment')
            .patchValue(
                this.detailTransaction?.rapport
                    ?.commmande_produit_accepte_comment
            );
    }
    ...
    OnIncrementButton(data: any) {
        let findProduct = this.detailTransaction?.detail_commande.find(
            (it) => it.id === data.id
        );
    ...
    public isAccepteForms(): boolean {
        return this.detailTransaction?.rapport?.provisionning_accepte ===
            'oui' ||
    ...
    public ShowAcceptedForm(): boolean {
        return this.transaction?.statut === StatutTransaction.TARITER ||
            (this.transaction?.statut === StatutTransaction.CLOTURER &&
                this.transaction?.traitement !==
    ...
    public IsCancel(): boolean {
        return this.transaction?.statut === StatutTransaction.SOUMIS &&
            this.transaction?.traitement === TraitementTransaction.EN_ENTENTE
            ? true
            : false;
    }
    public IsUpdate(): boolean {
        return (this.transaction?.statut === StatutTransaction.SOUMIS &&
            ...
            (this.transaction?.statut === StatutTransaction.TARITER &&
                this.transaction?.traitement === TraitementTransaction.REJETER)
            ? true
            : false;
    }
    IsReject() {
    ...
    public IsCloture(): boolean {
        return this.transaction?.statut === StatutTransaction.TARITER &&
            this.transaction?.traitement === TraitementTransaction.ACCEPTER
            ? true
            : false;
    }
    public IsShow(): boolean {
        return this.transaction?.statut === StatutTransaction.CLOTURER ||
            this.transaction?.traitement === TraitementTransaction.ACQUITER
            ? true
            : false;
    }
    ...
    public IsVerify(): boolean {
        return this.transaction?.operation === OperationTransaction.SWAP ||
            this.transaction?.operation === OperationTransaction.RESILIATION ||
            this.transaction?.operation === OperationTransaction.SUSPENSION ||
            this.transaction?.operation === OperationTransaction.VOLUME_DATA
            ? true
            ...
            if (result.value) {
                ...
                if (this.transaction?.operation === OperationTransaction.SWAP) {
                    ...
                    this.achatForm.patchValue({
                        detail_commande:
                            this.detailTransaction?.detail_commande,
                    });
                }
                const data = {
                    ...(this.transaction?.operation ===
            ...
            if (result.value) {
                this.supervisionOperationService
                    .OnCloseTransaction({
                        ...
                        ...(this.transaction?.operation ===
                        OperationTransaction.PROVISIONNING
                            ? { provisionning_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.ACHAT_SERVICE
                            ? {
                                  commmande_produit_cloture:
                                      this.selectedIsCloture,
                              }
                            : this.transaction?.operation ===
                              OperationTransaction.VOLUME_DATA
                            ? { volume_data_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.SWAP
                            ? { swap_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.RESILIATION
                            ? { resiliation_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.SUSPENSION
                            ? { suspension_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.CHANGEMENT_FORMULE
                            ? { chg_formule_cloture: this.selectedIsCloture }
                            : this.transaction?.operation ===
                              OperationTransaction.ACTIVATION
                            ? { activation_cloture: this.selectedIsCloture }
                            : null),
                    })
                    .subscribe({
                        next: (response) => {
