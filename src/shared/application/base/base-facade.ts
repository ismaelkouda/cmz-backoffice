...
> {
    ...
    /*     readonly loadingTreatment$: Observable<boolean> =
        this.loadingTreatmentSubject.asObservable();

    protected fetchValidateReportTreatment(
        fetchObservable: Observable<TEntity>
    ): void {
        if (this.loadingTreatmentSubject.getValue()) {
            return;
        }

        this.loadingTreatmentSubject.next(true);

        fetchObservable
            .pipe(
                debounceTime(PAGINATION_CONST.DEBOUNCE_TIME_MS),
                tap((response: TEntity) => {
                    if (this.isSimpleArrayResponse(response)) {
                ...
                finalize(() => this.loadingTreatmentSubject.next(false)),
                catchError((error: unknown) => {
                    const errorMessage = this.getErrorMessage(error);
                    this.toastService.error(errorMessage);
                    return throwError(() => error);
    ...
    private isSimpleArrayResponse(
        response: any
    ): response is SimpleArrayResponseDto<void> {
        return (
            response &&
