...
export class ContactFormComponent implements OnInit {
  ...
  public initForm(): void {
    ...
    const contactAdminTenantControl = this.adminForm.get('contact_admin_tenant');
    const emailAdminTenantControl = this.adminForm.get('email_admin_tenant');
    const usernameAdminTenantControl = this.adminForm.get('username_admin_tenant');
    adminTenantControl?.valueChanges.subscribe((value) => {
      if(!!value) {
        const userSelected = this.listUsers.find(user => user.id === value);
    ...
    const contactEscaladeTenantControl = this.adminForm.get('contact_escalade_tenant');
    const emailEscaladeTenantControl = this.adminForm.get('email_escalade_tenant');
    const usernameEscaladeTenantControl = this.adminForm.get('username_escalade_tenant');
    ...
    const gestionnaireTenantControl = this.adminForm.get('gestionnaire_tenant_id');
    const contactGestionnaireTenantControl = this.adminForm.get('contact_gestionnaire_tenant');
    const emailGestionnaireTenantControl = this.adminForm.get('email_gestionnaire_tenant');
    const usernameGestionnaireTenantControl = this.adminForm.get('username_gestionnaire_tenant');
    gestionnaireTenantControl?.valueChanges.subscribe((value) => {
      if(!!value) {
        console.log('value', value)
        const userSelected = this.listUsers.find(user => user.id === value);
        console.log('userSelected', userSelected)
        contactGestionnaireTenantControl?.setValue(userSelected?.contacts);
        emailGestionnaireTenantControl?.setValue(userSelected?.email);
        usernameGestionnaireTenantControl?.setValue(userSelected?.username);
      } else {
    ...
    this.settingService
      ...
      .subscribe(
        (response: any) => {
          ...
          console.log('this.listFormeJuridique', this.listFormeJuridique)
        },
        (error) => {
          this.toastService.error(error.error.message);
        }
  ...
  public handleSave() {
    this.telemetrieService
      .UpdateContactSla(formDataBuilder(this.adminForm.value)).subscribe({
        next: (response) => {
  ...
  public onFormPachValues(): void {
    ...
    this.adminForm.get('nom_tenant')?.patchValue(`${this.currentObject?.nom_tenant} [${this.currentObject?.tenant_code}]`);
    this.adminForm.get('adresse')?.patchValue(this.currentObject?.adresse);
    this.adminForm.get('compte_client')?.patchValue(this.currentObject?.compte_client);
    this.adminForm.get('contact_gestionnaire_tenant')?.patchValue(this.currentObject?.contact_gestionnaire_tenant);
    this.adminForm.get('email_gestionnaire_tenant')?.patchValue(this.currentObject?.email_gestionnaire_tenant);
    this.adminForm.get('username_gestionnaire_tenant')?.patchValue(this.currentObject?.username_gestionnaire_tenant);
    this.adminForm.get('contact_escalade_tenant')?.patchValue(this.currentObject?.contact_escalade_tenant);
    this.adminForm.get('email_escalade_tenant')?.patchValue(this.currentObject?.email_escalade_tenant);
    this.adminForm.get('username_escalade_tenant')?.patchValue(this.currentObject?.username_escalade_tenant);

    this.adminForm.get('admin_tenant_id')?.patchValue(this.currentObject?.admin_tenant_id);
    this.adminForm.get('contact_admin_tenant')?.patchValue(this.currentObject?.contact_admin_tenant);
    this.adminForm.get('email_admin_tenant')?.patchValue(this.currentObject?.email_admin_tenant);
    this.adminForm.get('username_admin_tenant')?.patchValue(this.currentObject?.username_admin_tenant);

    this.adminForm.get('numero_rccm')?.patchValue(this.currentObject?.numero_rccm);
    this.adminForm.get('form_juridique_code')?.patchValue(this.currentObject?.forme_juridique_code);
    this.adminForm.get('nom_gerant')?.patchValue(this.currentObject?.nom_gerant);
    this.adminForm.get('contact_gerant')?.patchValue(this.currentObject?.contact_gerant);
    this.adminForm.get('email_gerant')?.patchValue(this.currentObject?.email_gerant);

    this.adminForm.get('numero_cc')?.patchValue(this.currentObject?.numero_cc);
    this.adminForm.get('regime_code')?.patchValue(this.currentObject?.regime_code);
    this.adminForm.get('centre')?.patchValue(this.currentObject?.centre);

    this.currentObject?.["fichier_rccm"] ? this.adminForm.get('fichier_rccm')?.setValidators(Validators.required) : this.adminForm.get('fichier_rccm')?.clearValidators();
    this.currentObject?.["fichier_dfe"] ? this.adminForm.get('fichier_dfe')?.setValidators(Validators.required) : this.adminForm.get('fichier_dfe')?.clearValidators();
    this.currentObject?.["piece_gerant"] ? this.adminForm.get('piece_gerant')?.setValidators(Validators.required) : this.adminForm.get('piece_gerant')?.clearValidators();

  }

  public openModal(type: "fichier_rccm" | "fichier_dfe" | "piece_gerant"): void {
    switch (type) {
      case "fichier_rccm": window.open(this.currentObject?.["fichier_rccm"]); break;
      case "fichier_dfe": window.open(this.currentObject?.["fichier_dfe"]); break;
      case "piece_gerant": window.open(this.currentObject?.["piece_gerant"]); break;
    }
  }
  public onChangeFile(file: FileList, type: "fichier_rccm" | "fichier_dfe" | "piece_gerant") {
    switch (type) {
      case "fichier_rccm":
