...
export class FormsProfilComponent implements OnInit {
        ...
        this.telemetrieService
            ...
            .subscribe({
                next: (response) => {
                    ...
                    this.globalMetriquesEditRow = this.listAffectations
                        .filter((item) => {
        ...
        this.telemetrieService
            ...
            .subscribe({
                next: (response) => {
                    ...
                    this.globalMetriquesEditRow = this.listSms
                        .filter((item) => {
                            return item?.statut === 'actif';
                        })
                        .map((data) => {
    ...
    public onRowEditMetriqueSave(metrique: any) {
        ...
        if (data?.statut === 'inactif' || data?.seuil === null) {
            ...
            this.toastrService.warning(
                "Veuillez activez l'alarme ou Configurer le seuil"
            );
            return;
        } else {
            if (!checkValue) {
    ...
    isValidate(): boolean {
        return this.globalMetriquesEditRow.length === 0 && !this.adminForm.valid
            ? true
            : false;
    }
}
