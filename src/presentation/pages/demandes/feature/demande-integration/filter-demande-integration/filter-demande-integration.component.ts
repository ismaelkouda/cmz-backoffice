...
export class FilterDemandeIntegrationComponent implements OnInit {
    ...
    async GetAllUsers(): Promise<void> {
        ...
        this.listUsers = this.response.data.map((user: any) => {
            return { ...user, fullName: `${user.nom} ${user.prenoms}` };
        });
    }

    public onSubmitFilterForm(): void {
        const date_debut = moment(
            this.filterForm.get('date_debut').value
        ).isValid()
            ? this.filterForm.get('date_debut').value
            : null;
        const date_fin = moment(this.filterForm.get('date_fin').value).isValid()
            ? this.filterForm.get('date_fin').value
            : null;
