import { SLA_AGREEMENTS_STATUS_ENUM, T_SLA_AGREEMENTS_STATUS_ENUM } from './../../../data-access/sla-agreements/enums/sla-agreements-status.enum';
...
export class TableSlaAgreementsComponent {
    ...
    constructor(private tableExportExcelFileService: TableExportExcelFileService,
        private slaAgreementsApiService: SlaAgreementsApiService) { }

    public onExportExcel(): void {
        this.listSlaAgreements$.subscribe(data => {
            if (data) { this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "List_sla_agreements"); }
        });
    }
    ...
    public getStatusSlaAgreementsBadge(selectedSlaAgreements?: { statut: T_SLA_AGREEMENTS_STATUS_ENUM }): TYPE_COLOR_STATUS_BADGE {
            if (!selectedSlaAgreements || !selectedSlaAgreements.statut) { return 'badge-success'; }
    
            const etapeMap: Record<T_SLA_AGREEMENTS_STATUS_ENUM, TYPE_COLOR_STATUS_BADGE> = {
