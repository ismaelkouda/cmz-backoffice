...
import { MunicipalitiesFindoneFacade } from '@presentation/pages/administrative-boundary/core/application/services/municipalities/municipalities-findone.facade';
...
import { MunicipalitiesFormControl } from '@presentation/pages/administrative-boundary/core/domain/controls/municipalities/municipalities-form.control';
...
export class MunicipalitiesFormComponent implements OnInit {
    ...
    private readonly findOneFacade = inject(MunicipalitiesFindoneFacade);
    ...
    readonly currentMunicipality = toSignal(this.findOneFacade.item$, { initialValue: null });
    ...
    public form: FormGroup<MunicipalitiesFormControl> = this.fb.group<MunicipalitiesFormControl>({
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (municipality && Object.keys(municipality).length > 0) {
    ...
    public getErrorMessage(fieldName: string): string {
        ...
        if (errors['minlength']) {
            return `${this.translate.instant('ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.FORM.VALIDATION.MIN_LENGTH')}: ${errors['minlength'].requiredLength}`;
        }
        if (errors['maxlength']) {
            return `${this.translate.instant('ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.FORM.VALIDATION.MAX_LENGTH')}: ${errors['maxlength'].requiredLength}`;
        }
        if (errors['pattern']) {
            return this.translate.instant('ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.FORM.VALIDATION.INVALID_FORMAT');
        }
        return this.translate.instant('ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.FORM.VALIDATION.INVALID_INPUT');
    }
    ...
    private getSweetAlertTitle(): string {
        return this.isEditMode() ? 'ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.SWEET_ALERT.TITLE_UPDATE' : 'ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.SWEET_ALERT.TITLE_CREATE';
    }

    private getSweetAlertMessage(): string {
        return this.isEditMode() ? 'ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.SWEET_ALERT.MESSAGE_UPDATE' : 'ADMINISTRATIVE_BOUNDARY.MUNICIPALITIES.SWEET_ALERT.MESSAGE_CREATE';
    }
    ...
    onCancel(): void {
        this.router.navigate([ADMINISTRATIVE_BOUNDARY_ROUTE + '/' + MUNICIPALITIES_ROUTE]);
    }
}
