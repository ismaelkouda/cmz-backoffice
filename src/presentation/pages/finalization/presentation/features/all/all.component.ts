...
export class AllComponent {
    ...
    public readonly headerButtons = computed<TableHeaderButton[]>(() => [
        {
            ...
            tooltip: this.t('FINALIZATION.ALL.TOOLTIP.REFRESH'),
        },
        {
            label: 'COMMON.EXPORT',
            actionId: 'export',
    ...
    private readonly exportTooltip = computed(() => {
        ...
        if (permission) {
            return this.t('FINALIZATION.ALL.TOOLTIP.NO_PERMISSION_EXPORT');
        }
        if (noData) {
            return this.t('FINALIZATION.ALL.TOOLTIP.NO_EXPORT');
        }
        return this.t('FINALIZATION.ALL.TOOLTIP.EXPORT').replace(
            '{nb}',
