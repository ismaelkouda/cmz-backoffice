...
export class TableAllComponent implements OnDestroy {
    ...
    public getAllTooltip(uniqId: string): string {
        const allLabel = this.translate.instant(
            'FINALIZATION.ALL.TABLE.SEE_MORE'
        );
    ...
    getOperatorLabel(operator: string): string {
        ...
        const translationMap: Record<string, string> = {
            orange: 'FINALIZATION.ALL.OPTIONS.OPERATORS.ORANGE',
            mtn: 'FINALIZATION.ALL.OPTIONS.OPERATORS.MTN',
            moov: 'FINALIZATION.ALL.OPTIONS.OPERATORS.MOOV',
        };
        const key = translationMap[normalized];
        return key ? this.translate.instant(key) : operator;
    }

    public getStateSeverity(state: ReportState): TagSeverity {
