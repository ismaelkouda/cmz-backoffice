...
export class ThirdLevelComponent implements OnInit {
  ...
  public listParentLevelDatas: Array<any> = [];
  ...
  public GellAllParenttLevel() {
    this.settingService
      .getAllExploiatations({})
      .subscribe({
        next: (response) => {
          this.listParentLevelDatas = response['data'];
        },
        error: (error) => {
          this.toastrService.error(error.message);
        }
  ...
  onEditForm(item: any, content: any) {
    ...
    // this.adminForm.get('niveau_deux_id').patchValue(item?.niveau_deux_id);
    this.adminForm.enable();
  }
