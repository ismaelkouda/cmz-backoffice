...
export class FormFirstLevelComponent implements OnInit {
    ...
    constructor(
        ...
        this.firstLevelMenus =
            this.mappingService.structureGlobale?.niveau_1_menu;
        this.secondLevelMenus =
            this.mappingService.structureGlobale?.niveau_2_menu;
    }
    ngOnInit() {
        if (!this.currentObject) {
        } else if (
            this.currentObject?.type === 'visualiser' ||
            this.currentObject?.type === 'show'
        ) {
    ...
    public GetAllGroupes() {
        this.settingService.getAllDirectionRegionales({}, 1).subscribe({
            next: (response) => {
