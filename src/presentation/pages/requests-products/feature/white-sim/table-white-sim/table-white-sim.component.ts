...
type ModalAction = {
    data: Folder;
    action: 'view-white-sim' | 'journal-white-sim';
    view: 'modal';
};
const INIT_TYPE_TREATMENT: TreatmentDemands = {
    module: 'requests-products',
    abandonner: false,
    modifier: false,
    visualiser: false,
    cloturer: false,
...
export class TableWhiteSimComponent {
    ...
    getStepCommandWhiteSimBadge(commandWhiteSim?: {
    ...
    getStateCommandWhiteSimBadge(commandWhiteSim?: {
        statut?: T_BADGE_ETAPE;
        traitement?: T_BADGE_ETAT;
    }): TYPE_COLOR_ETAT_BADGE {
        if (
            !commandWhiteSim ||
            !commandWhiteSim.statut ||
            !commandWhiteSim.traitement
        ) {
            return 'badge-dark';
        }
        ...
        return (
            stateMap[commandWhiteSim.statut]?.[commandWhiteSim.traitement] ||
            'badge-dark'
        );
    }
    ...
    public handleAction(params: Action): void {
        ...
        switch (params.view) {
            case 'modal':
                if (params.action === 'view-white-sim') {
                    this.handleCommandWhiteSimTreatment(params.data);
                }
                if (params.action === 'journal-white-sim') {
    ...
    handleCommandWhiteSimTreatment(commandWhiteSim: {
        ...
        this.typeTreatment =
            this.getTreatmentButtonViewCommandStyle(
                commandWhiteSim
            )?.typeTreatment;
    }
    ...
    handleJournal(commandWhiteSim: { numero_demande: string }): void {
        ...
        modalRef.componentInstance.numero_demande =
            commandWhiteSim?.numero_demande;
    ...
    getTreatmentButtonPaymentStyle(commandWhiteSim: {
        ...
        if (
            commandWhiteSim?.statut === BADGE_ETAPE.CLOTURE &&
            commandWhiteSim.traitement === BADGE_ETAT.ABANDONNE
        ) {
            return createButtonStyle(
                'p-button-secondary',
                'pi pi-print',
