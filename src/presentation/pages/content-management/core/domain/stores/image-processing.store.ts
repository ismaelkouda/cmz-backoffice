...
import { CropRegion, ImageProcessingError, ProcessingOptions, ProcessingResult, ProcessingStage } from '../types/image-processing.types';
...
export class ImageProcessingStore extends signalStore(
    ...
    withComputed(({ stage, progress, currentResult, error }) => ({
        ...
        compressionRatio: computed(() => currentResult()?.metadata.compressionRatio || 0),
        processedSize: computed(() => currentResult()?.metadata.processed.size || 0)
    })),
    withMethods((store,
        ...
        return {
            ...
            processImage: rxMethod<{
                ...
                pipe(
                    ...
                    switchMap(({ image, cropRegion, options, useWorker = true }) => {
                        if (useWorker && typeof Worker !== 'undefined') {
                            ...
                            return workerService.processInWorker$(imageData, cropRegion, options).pipe(
                                tap(result => setResult(result)),
                                catchError(error => {
                                    return coreService.cropImage$(image, cropRegion, options).pipe(
                                        tap(result => setResult(result)),
                                        catchError(workerError => {
                                            setError(workerError as ImageProcessingError);
                                            return of(null);
                                        })
                                    );
                                })
                    ...
                    switchMap(({ file, cropRegion, options }) => {
                        ...
                        return coreService.loadImage$(file).pipe(
                            switchMap(image => {
                                ...
                                const safeOptions = { ...options, format: options?.format || 'image/webp' } as ProcessingOptions;
                                ...
                                if (typeof Worker !== 'undefined') {
                                    ...
                                    return workerService.processInWorker$(imageData, cropRegion, safeOptions).pipe(
                                        tap(result => setResult(result)),
                                        catchError(error => {
                                            return coreService.cropImage$(image, cropRegion, safeOptions).pipe(
                                                tap(result => setResult(result)),
                                                catchError(workerError => {
                                                    setError(workerError as ImageProcessingError);
                                                    return of(null);
