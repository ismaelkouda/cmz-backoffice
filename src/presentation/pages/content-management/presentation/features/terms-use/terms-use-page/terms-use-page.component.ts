...
export class TermsUsePageComponent implements OnInit {
    ...
    readonly filterFields: Signal<FilterField[]> = computed(() => {
        ...
        return [
            ...
            },
            {
                ...
                label: this.t('CONTENT_MANAGEMENT.TERMS_USE.FILTER.VERSION'),
                placeholder: this.t(
                    'CONTENT_MANAGEMENT.TERMS_USE.FILTER.VERSION_PLACEHOLDER'
                ),
                icon: 'pi pi-search',
                translationKeys: {
                    label: 'CONTENT_MANAGEMENT.TERMS_USE.FILTER.VERSION',
                    placeholder:
                        'CONTENT_MANAGEMENT.TERMS_USE.FILTER.VERSION_PLACEHOLDER',
                },
            },
    ...
    readonly form = this.fb.group<TermsUseFilterControl>({
    ...
    ngOnInit(): void {
        this.title.setTitle(this.t('CONTENT_MANAGEMENT.TERMS_USE.PAGE_TITLE'));

        this.translate.onLangChange
            .pipe(takeUntilDestroyed(this.destroyRef))
            .subscribe(() => {
                this.title.setTitle(
                    this.t('CONTENT_MANAGEMENT.TERMS_USE.PAGE_TITLE')
                );
            });
    }
    ...
    public onNavigateToForm(event: {
        ...
        const queryParams = event.item
            ...
            : { ref: event.ref };
        this.router.navigate([TERMS_USE_FORM], {
    ...
    public onDeleteClicked(item: TermsUseEntity): void {
        ...
        SweetAlert.fire({
            ...SWEET_ALERT_PARAMS,
            title: this.t(
                'CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.TITLE_DELETE'
            ),
            text: `${this.t('CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.MESSAGE_DELETE')}`,
    ...
    public onEnableClicked(item: TermsUseEntity): void {
        if (!item.uniqId) {
            return;
        }
        SweetAlert.fire({
            ...SWEET_ALERT_PARAMS,
            title: this.t(
                'CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.TITLE_ENABLE'
            ),
            text: `${this.t('CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.MESSAGE_ENABLE')}`,
            backdrop: false,
            confirmButtonText: this.t('COMMON.CONFIRM'),
            cancelButtonText: this.t('COMMON.CANCEL'),
        }).then((result) => {
    ...
    public onDisableClicked(item: TermsUseEntity): void {
        if (!item.uniqId) {
            return;
        }
        SweetAlert.fire({
            ...SWEET_ALERT_PARAMS,
            title: this.t(
                'CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.TITLE_DISABLE'
            ),
            text: `${this.t('CONTENT_MANAGEMENT.TERMS_USE.SWEET_ALERT.MESSAGE_DISABLE')}`,
            confirmButtonText: this.t('COMMON.CONFIRM'),
            cancelButtonText: this.t('COMMON.CANCEL'),
        }).then((result) => {
            if (result.isConfirmed) {
