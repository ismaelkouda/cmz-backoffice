...
import { LEGAL_NOTICE_TABLE_CONST } from '@presentation/pages/content-management/core/domain/constants/legal-notice/legal-notice-table.constants';
...
@Component({
    selector: 'app-table-legal-notice',
    standalone: true,
    templateUrl: './table-legal-notice.component.html',
    styleUrls: ['./table-legal-notice.component.scss'],
...
export class TableLegalNoticeComponent implements OnDestroy {
    ...
    readonly legalNoticeData = signal<LegalNoticeEntity[]>([]);
    readonly isLoading = signal<boolean>(false);
    readonly hasData = signal<boolean>(false);

    @Input({ required: true })
    set legalNotice$(value: Observable<LegalNoticeEntity[]>) {
        this._legalNotice$ = value;
        this._subscribeToData();
    }

    @Input({ required: true })
    pagination$!: Observable<Paginate<LegalNoticeEntity>>;
    ...
    private _legalNotice$!: Observable<LegalNoticeEntity[]>;

    @Output() createRequested = new EventEmitter<void>();
    @Output() editRequested = new EventEmitter<LegalNoticeEntity>();
    @Output() deleteRequested = new EventEmitter<LegalNoticeEntity>();
    @Output() refreshRequested = new EventEmitter<void>();
    @Output() viewRequested = new EventEmitter<LegalNoticeEntity>();
    @Output() publishRequested = new EventEmitter<LegalNoticeEntity>();
    @Output() unpublishRequested = new EventEmitter<LegalNoticeEntity>();

    private readonly exportFilePrefix = this.normalizeExportPrefix(
        this.appCustomizationService.config.app.name
    );

    readonly tableConfig: TableConfig = LEGAL_NOTICE_TABLE_CONST;
    ...
    public onExportExcel(): void {
        const data = this.legalNoticeData();
    ...
    private _subscribeToData(): void {
        this._legalNotice$
            .pipe(
                ...
                tap((data) => {
                    const items = data ?? [];
                    this.legalNoticeData.set(items);
    ...
    trackByUniqId(_: number, item: LegalNoticeEntity): string {
        return item.uniqId;
    }
