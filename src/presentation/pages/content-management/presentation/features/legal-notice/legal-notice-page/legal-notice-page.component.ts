...
export class LegalNoticePageComponent implements OnInit {
    ...
    readonly filterFields: Signal<FilterField[]> = computed(() => {
        ...
        return [
            ...
            },
            {
                ...
                label: this.t('CONTENT_MANAGEMENT.LEGAL_NOTICE.FILTER.VERSION'),
                placeholder: this.t(
                    'CONTENT_MANAGEMENT.LEGAL_NOTICE.FILTER.VERSION_PLACEHOLDER'
                ),
                icon: 'pi pi-search',
                translationKeys: {
                    label: 'CONTENT_MANAGEMENT.LEGAL_NOTICE.FILTER.VERSION',
                    placeholder:
                        'CONTENT_MANAGEMENT.LEGAL_NOTICE.FILTER.VERSION_PLACEHOLDER',
                },
            },
    ...
    readonly form = this.fb.group<LegalNoticeFilterControl>({
    ...
    ngOnInit(): void {
        this.title.setTitle(
            this.t('CONTENT_MANAGEMENT.LEGAL_NOTICE.PAGE_TITLE')
        );
    ...
    public onNavigateToForm(event: {
        ...
        const queryParams = event.item
            ...
            : { ref: event.ref };
        this.router.navigate([LEGAL_NOTICE_FORM], {
    ...
    public onEnableClicked(item: LegalNoticeEntity): void {
        if (!item.uniqId) {
            return;
        }
        SweetAlert.fire({
    ...
    public onDisableClicked(item: LegalNoticeEntity): void {
        if (!item.uniqId) {
            return;
        }
        SweetAlert.fire({
