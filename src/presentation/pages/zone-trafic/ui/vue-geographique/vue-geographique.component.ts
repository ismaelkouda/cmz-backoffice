...
export class VueGeographiqueComponent implements OnInit {
    ...
    public title =
        'Zone géographique - Système de Gestion de Collecte Centralisée';
    ...
    public GetAllZOneTrafic() {
        this.zoneTraficService.GetAllZOneTrafic({}, this.p).subscribe({
            next: (response) => {
    ...
    public GetAllDepartements() {
        this.zoneTraficService.GetAllDepartements({}).subscribe({
            next: (response) => {
    ...
    onChangeFirstLvel(event: any) {
        ...
        this.listCommunes = this.selectedDepartement?.communes.map(
            (element) => {
                return { ...element, fullName: `${element.libelle}` };
            }
        );
    ...
    GetPositionSimGeojson(id: number) {
        this.zoneTraficService.GetPositionSimGeojson(id).subscribe({
            next: (response) => {
                this.zoneId = id;
    ...
    public disableAction(): boolean {
        return this.listZonesTrafics === undefined ||
            this.listZonesTrafics?.length === 0
            ? true
            : false;
    }
