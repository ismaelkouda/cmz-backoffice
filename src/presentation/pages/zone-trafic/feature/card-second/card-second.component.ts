...
export class CardSecondComponent implements AfterViewInit, OnDestroy {
    ...
    initMap() {
        ...
        this.map.setView(
            new L.LatLng(
                this.datas.site_geo.features[0]?.properties.LONGITUDE,
                this.datas.site_geo.features[0]?.properties.LATITUDE
            ),
            16
        );
    ...
    onMapReady() {
        const firstLevelLibelle =
            this.mappingService.structureGlobale?.niveau_1;
        const secondLevelLibelle =
            this.mappingService.structureGlobale?.niveau_2;
        const thirdLevelLibelle =
        ...
        const geoJsonSite = L.geoJSON(this.datas.site_geo, {
            ...
            onEachFeature: function (feature, layer) {
                layer.bindTooltip(feature.properties.NAME, {
                    permanent: true,
                    direction: 'bottom',
                    className: 'leaflet-tooltip-other-points',
                });
            },
        });
                ...
                layer
                    .bindPopup(
                        '<div>' +
                            ...
                            ':</strong>' +
                            '<span>' +
                            feature?.properties?.niveau_un?.nom +
                            '</span>' +
                            '<br>' +
                            '<strong>' +
                            secondLevelLibelle +
                            ':</strong>' +
                            '<span>' +
                            feature?.properties?.niveau_deux?.nom +
                            '</span>' +
                            '<br>' +
                            '<strong>' +
                            thirdLevelLibelle +
                            ' :</strong>' +
                            '<span>' +
                            feature?.properties?.niveau_trois?.nom +
                            '</span>' +
                            '<br>' +
                            '<strong>' +
                            'Statut :' +
                            ...
                            '</strong>' +
                            '<span>' +
                            feature?.properties?.point_emplacement +
                            '</span>' +
                            '<br>' +
                            '<strong>' +
                            'Site :' +
                            ...
                            '<span>' +
                            (
                                Number(feature?.properties?.solde_actuel_go) ||
                                0
                            ).toFixed(2) +
                            '</span>' +
                            '<br>' +
        ...
        const geojsonRessort = L.geoJSON(this.datas.ressort, {
            ...
            onEachFeature: function (feature, layer) {
                ...
                layer.bindTooltip(feature.properties.RESSORT, {
                    permanent: true,
                    direction: 'center',
                    className: 'leaflet-tooltip-ressort',
                });
            },
        });
                    ...
                    return L.marker(latlng, {
                        ...
                        .bindPopup(
                            '<div>' +
                                ...
                                ':</strong>' +
                                '<span>' +
                                feature?.properties?.sim?.niveau_un?.nom +
                                '</span>' +
                                '<br>' +
                                '<strong>' +
                                secondLevelLibelle +
                                ':</strong>' +
                                '<span>' +
                                feature?.properties?.sim?.niveau_deux?.nom +
                                '</span>' +
                                '<br>' +
                                '<strong>' +
                                thirdLevelLibelle +
                                ' :</strong>' +
                                '<span>' +
                                feature?.properties?.sim?.niveau_trois?.nom +
                                '</span>' +
                                '<br>' +
                                '<strong>' +
                                'Statut :' +
                                ...
                                '</strong>' +
                                '<span>' +
                                feature?.properties?.sim?.point_emplacement +
                                '</span>' +
                                '<br>' +
                                '<strong>' +
                                'Site :' +
                                ...
                                '</strong>' +
                                '<span>' +
                                feature?.properties?.long_site +
                                ', ' +
                                feature?.properties?.lat_site +
                                '</span>' +
                                '<br>' +
                                '<strong>' +
                                'Solde Data (Go) :' +
                                '</strong>' +
                                "<span style='color: #FFFFFF;font-weight: bold;background-color: #27ae60; padding: 2px 10px; border-radius: 2px'>" +
                                (
                                    Number(
                                        feature?.properties?.solde_actuel_go
                                    ) || 0
                                ).toFixed(2) +
                                '</span>' +
                                '<br>' +
                    ...
                    return L.marker(latlng, {
                        ...
                        .bindPopup(
                            '<div>' +
                                ...
                                'Solde Data (Go) :' +
                                '</strong>' +
                                "<span style='color: #000000;font-weight: bold;background-color: #FFFF00; padding: 2px 10px; border-radius: 2px'>" +
                                (
                                    Number(
                                        feature?.properties?.solde_actuel_go
                                    ) || 0
                    ...
                    return L.marker(latlng, {
                        ...
                        .bindPopup(
                            '<div>' +
                                ...
                                'Solde Data (Go) :' +
                                '</strong>' +
                                "<span style='color: #FFFFFF;font-weight: bold;background-color: #FE9A2E; padding: 2px 10px; border-radius: 2px'>" +
                                (
                                    Number(
                                        feature?.properties?.solde_actuel_go
                                    ) || 0
                    ...
                    return L.marker(latlng, {
                        ...
                        .bindPopup(
                            '<div>' +
                                ...
                                'Solde Data (Go) :' +
                                '</strong>' +
                                "<span style='color: #FFFFFF;font-weight: bold;background-color: #e74c3c; padding: 2px 10px; border-radius: 2px'>" +
                                (
                                    Number(
                                        feature?.properties?.solde_actuel_go
                                    ) || 0
        ...
        var layerGeoJson = {
            "<span style='font-weight:bold;' ><b>SITES - OCI</b></span><span><img src='assets/svg/oci_site.svg' style='width: 12px; margin-left: 20px;'/></span>":
                geoJsonSite,
            "<span style='font-weight:bold'><b>EMPLACEMENTS</b></span><span><img src='assets/svg/sim_loc_noir.svg' style='width: 10px; margin-left: 20px;'/></span>":
                geoJsonSim,
            "<span style='font-weight:bold'><b>RESSORTS</b></span><span><img src='assets/svg/moins.png' style='width: 30px;height: 15px; margin-left: 20px;'/></span>":
                geojsonRessort,
            "<span style='font-weight:bold;' ><b>SIM état Normale</b></span><span><img src='assets/svg/sim_loc_vert.svg' style='width: 10px; margin-left: 20px; color: #2F02FB;'/></span>":
                normalLayerGroup,
            "<span style='font-weight:bold;' ><b>Alarmes Mineures</b></span><span><img src='assets/svg/sim_loc_jaune.svg' style='width: 10px; margin-left: 20px; color: #2F02FB;'/></span>":
                mineurLayerGroup,
            "<span style='font-weight:bold;' ><b>Alarmes Majeures</b></span><span><img src='assets/svg/sim_loc_orange.svg' style='width: 10px; margin-left: 20px; color: #2F02FB;'/></span>":
                majeurLayerGroup,
            "<span style='font-weight:bold;' ><b>Alarmes Critiques</b></span><span><img src='assets/svg/sim_loc_rouge.svg' style='width: 10px; margin-left: 20px; color: #2F02FB;'/></span>":
                critiqueLayerGroup,
        };
    ...
    GetPositionSimTracking(
        normalLayer: any = [],
        mineurLayer: any = [],
        majeurLayer: any = [],
        critiqueLayer: any = []
    ) {
        this.zoneTraficService.GetPositionSimTracking(this.zoneId).subscribe({
            next: (response) => {
                ...
                L.geoJSON(this.currentTraficLayer, {
                    pointToLayer: function (feature, latlng) {
                        if (feature?.properties?.Alarme === TypeAlarme.NORMAL) {
                            ...
                            feature?.properties?.Alarme === TypeAlarme.MINEUR
                        ) {
                            mineurLayer.push(feature);
                        } else if (
                            feature?.properties?.Alarme === TypeAlarme.MAJEUR
                        ) {
                            majeurLayer.push(feature);
                        } else if (
                            feature?.properties.Alarme === TypeAlarme.CRITIQUE
                        ) {
