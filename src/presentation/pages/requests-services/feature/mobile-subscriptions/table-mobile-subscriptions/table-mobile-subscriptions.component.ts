...
export class TableMobileSubscriptionsComponent {
  ...
  public pageCallback() {
    this.sharedService.fetchDemands({operation: OperationTransaction.ACTIVATION} as Folder);
  }

  public onExportExcel(): void {
    this.listDemands$.subscribe(data => {
      if (data) { this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "List_demands"); }
    });
  }
  ...
  getStateBadge(dossier?: { statut?: T_BADGE_ETAPE; traitement?: T_BADGE_ETAT }): TYPE_COLOR_ETAT_BADGE {
  ...
  public handleAction(params: Action): void {
    ...
    switch (params.view) {
      case 'modal':
        if (params.action === 'view-mobile-subscription') { this.handleDossierTreatment(params.data) }
        if (params.action === 'journal-mobile-subscription') { this.handleJournal(params.data) };
        break;
      ...
      case 'page':
        ...
        if (params.action === 'simple-add-mobile-subscription') { this.interfaceUser.emit(params) };
        break;
    }
  }

  handleDossierTreatment(dossier: { statut: string, traitement: string }): void {
    this.visibleFormDossier = true;
    this.typeTreatment = this.getTreatmentButtonViewStyle(dossier)?.typeTreatment;
  }
  ...
  getTreatmentButtonOpenFolderStyle(dossier: { statut: string, traitement: string, numero_demande: string }): { class: string, icon: string, tooltip: string} {
