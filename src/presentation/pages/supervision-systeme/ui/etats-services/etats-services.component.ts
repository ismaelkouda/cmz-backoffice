...
export class EtatsServicesComponent implements OnInit {
    ...
    public title =
        'Etats services - Système de Gestion de Collecte Centralisée';
    constructor(
    ...
    public GetAllEtatServices() {
        this.supervisionSystemeService.GetAllEtatServices({}).subscribe({
            next: (response) => {
    ...
    onChangeItem(event: any) {
        ...
        this.listExploitations = this.selectedDirection?.exploitations.map(
            (element) => {
                return {
                    ...element,
