...
import { FetchResourcesUseCase } from '../use-cases/resources/fetch-resources.use-case';

@Injectable({
    providedIn: 'root',
})
export class ResourcesFacade extends SimpleBaseFacade<
    ResourcesEntity,
    undefined
> {
    private readonly fetchResourcesUseCase = inject(FetchResourcesUseCase);
    ...
    fetchResources(forceRefresh = false): void {
        if (!this.shouldFetch(forceRefresh)) {
            return;
        }
        const fetch = this.fetchResourcesUseCase.execute();
