...
import {
    smsBalanceStatusApiResponseInterface,
    smsBalanceStatusInterface,
} from '../interfaces/sms-balance-status.interface';
...
export class smsBalanceStatusApiService {
    ...
    private smsBalanceStatusSubject = new BehaviorSubject<
        Array<smsBalanceStatusInterface>
    >([]);
    private smsBalanceStatusPagination = new BehaviorSubject<
        Paginate<smsBalanceStatusInterface>
    >({} as Paginate<smsBalanceStatusInterface>);
    private smsBalanceStatusSelected =
        new BehaviorSubject<smsBalanceStatusInterface>(
            {} as smsBalanceStatusInterface
        );
    private loadingSmsBalanceStatusSubject = new BehaviorSubject<boolean>(
        false
    );
    private dataFilterSmsBalanceStatusSubject =
        new BehaviorSubject<smsBalanceStatusFilterInterface>(
            {} as smsBalanceStatusFilterInterface
        );
    private dataNbrPageSmsBalanceStatusSubject = new BehaviorSubject<string>(
        '1'
    );
    private apiResponseSmsBalanceStatusSubject =
        new BehaviorSubject<smsBalanceStatusApiResponseInterface>(
            {} as smsBalanceStatusApiResponseInterface
        );

    fetchSmsBalanceStatus(
    ...
    getSmsBalanceStatusPagination(): Observable<
        Paginate<smsBalanceStatusInterface>
    > {
    ...
    setSmsBalanceStatusSelected(
        smsBalanceStatus: smsBalanceStatusInterface
    ): void {
