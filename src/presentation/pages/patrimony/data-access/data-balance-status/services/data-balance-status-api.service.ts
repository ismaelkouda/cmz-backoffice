...
import { dataBalanceStatusApiResponseInterface, dataBalanceStatusInterface } from '../interfaces/data-balance-status.interface';
...
export class dataBalanceStatusApiService {
    ...
    private dataBalanceStatusSubject = new BehaviorSubject<Array<dataBalanceStatusInterface>>([]);
    private dataBalanceStatusPagination = new BehaviorSubject<Paginate<dataBalanceStatusInterface>>({} as Paginate<dataBalanceStatusInterface>);
    private dataBalanceStatusSelected = new BehaviorSubject<dataBalanceStatusInterface>({} as dataBalanceStatusInterface);
    private loadingDataBalanceStatusSubject = new BehaviorSubject<boolean>(false);
    private dataFilterDataBalanceStatusSubject = new BehaviorSubject<dataBalanceStatusFilterInterface>({} as dataBalanceStatusFilterInterface);
    private dataNbrPageDataBalanceStatusSubject = new BehaviorSubject<string>('1');
    private apiResponseDataBalanceStatusSubject = new BehaviorSubject<dataBalanceStatusApiResponseInterface>({} as dataBalanceStatusApiResponseInterface);
    
    fetchDataBalanceStatus(data: dataBalanceStatusFilterInterface, nbrPage: string = '1'): void {
        ...
        const url: string = dataBalanceStatusEndpointEnum.PATRIMONY_SIM_SIMS_ALL_PAGE.replace('{page}', nbrPage);
    ...
    getDataBalanceStatusPagination(): Observable<Paginate<dataBalanceStatusInterface>> {
    ...
    setDataBalanceStatusSelected(dataBalanceStatus: dataBalanceStatusInterface): void {
