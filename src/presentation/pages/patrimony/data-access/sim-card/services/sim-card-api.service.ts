...
import {
    simCardApiResponseInterface,
    simCardInterface,
} from '../interfaces/sim-card.interface';
...
export class simCardApiService {
    ...
    private dataFilterSimCardSubject =
        new BehaviorSubject<simCardFilterInterface>(
            {} as simCardFilterInterface
        );
    private dataNbrPageSimCardSubject = new BehaviorSubject<string>('1');
    private apiResponseSimCardSubject =
        new BehaviorSubject<simCardApiResponseInterface>(
            {} as simCardApiResponseInterface
        );
    ...
    private simCardDetailsSubject =
        new BehaviorSubject<simCardDetailsInterface>(
            {} as simCardDetailsInterface
        );
    ...
    fetchSimCardDetails(imsi: string): void {
        ...
        const url: string =
            simCardEndpointEnum.POST_PATRIMOINE_SIM_SIMS_imsi_Details.replace(
                '{imsi}',
                imsi
            );
