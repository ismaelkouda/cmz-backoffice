...
type PageAction = {
    ...
    action: 'view-white-sim-card';
    view: 'page';
};
type ModalAction = {
    data: whiteSimCardInterface;
    action: 'journal-white-sim-card';
    view: 'modal';
};
...
export class TableWhiteSimCardComponent {
    ...
    getStatusWhiteSimCardBadge(selectedWhiteSimCard?: {
        statut: T_WHITE_SIM_CARD_STATUS_ENUM;
    }): TYPE_COLOR_STATUS_BADGE {
        if (!selectedWhiteSimCard || !selectedWhiteSimCard.statut) {
    ...
    private onSelectWhiteSimCard(
        selectedWhiteSimCard: whiteSimCardInterface
    ): void {
        this.whiteSimCardSelected = selectedWhiteSimCard;
        this.whiteSimCardApiService.setWhiteSimCardSelected(
            selectedWhiteSimCard
        );
    }
    ...
    public handleAction(params: Action): void {
        ...
        switch (params.view) {
            case 'modal':
                if (params.action === 'journal-white-sim-card') {
                    this.handleWhiteSimCardJournal(params.data);
                }
                break;

            case 'page':
                if (params.action === 'view-white-sim-card') {
    ...
    public handleWhiteSimCardJournal(
        selectedWhiteSimCard: whiteSimCardInterface
    ): void {
