...
export class FilterWhiteSimCardDetailsComponent implements OnInit, OnDestroy {
    ...
    public initFormFilter(): void {
        this.whiteSimCardApiService.getDataFilterWhiteSimCardDetails().pipe(takeUntil(this.destroy$)).subscribe((filterData) => {
            this.formFilter = this.fb.group<whiteSimCardDetailsFilterInterface>({
                imsi: new FormControl<string>(filterData?.["imsi"], { nonNullable: true }) ,
