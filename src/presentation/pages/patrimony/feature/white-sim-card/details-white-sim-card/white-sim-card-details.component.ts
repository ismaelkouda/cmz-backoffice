...
export class WhiteSimCardDetailsComponent implements OnInit, OnDestroy {
    ...
    private getParamsInUrl(): void {
        if (!isTypeView(this.urlParamRef) || !this.urlParamId) {
            ...
            if (this.urlParamId) {
                this.listWhiteSimCardDetails$ = this.whiteSimCardApiService.getWhiteSimCardDetails();
                this.whiteSimCardApiService.fetchWhiteSimCardDetails({ id: this.urlParamId });
                this.whiteSimCardApiService.isLoadingWhiteSimCardDetails().pipe(takeUntil(this.destroy$)).subscribe((spinner: boolean) => {
