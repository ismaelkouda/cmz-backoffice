...
import { SIM_CARD_IDENTIFICATION_ENUM, T_SIM_CARD_IDENTIFICATION_ENUM } from './../../../data-access/sim-card/enums/sim-card-identification.enum';
...
type ModalAction = { data: simCardInterface, action: 'map-sim-card' | 'qr-code-sim-card', view: 'modal' };
type TYPE_COLOR_IDENTIFICATION_BADGE = 'badge-success' | 'badge-warning' | 'badge-danger';
type TYPE_COLOR_STATUS_BADGE = 'badge-warning' | 'badge-dark' | 'badge-success' | 'badge-danger';
type ButtonMapSimCardStyle = { disabled: boolean, tooltip: string };
type ButtonMapSimCardData = { latitude: string, lat_reseau: string, date_localisation: string, msisdn: string };
...
export class TableSimCardComponent {
    ...
    private openStreetMap = L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: 'PATRIMOINE SIM-MAP', detectRetina: false, maxNativeZoom: 19,
        maxZoom: 23, minZoom: 12, noWrap: false, opacity: 1, subdomains: 'abc', tms: false,
    })
    private satellite = L.tileLayer('http://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', {
        maxZoom: 23, minZoom: 10, subdomains: ['mt0', 'mt1', 'mt2', 'mt3'], attribution: 'PATRIMOINE SIM-MAP',
    })
    private traficIcon = L.icon({
        iconUrl: '../../../../../assets/svg/sim_loc_noir.svg',
        iconSize: [45, 45], iconAnchor: [17, 17]
    });
    ...
    constructor(private toastService: ToastrService, private clipboardService: ClipboardService, private ngbModal: NgbModal,
        private simCardApiService: simCardApiService, private tableExportExcelFileService: TableExportExcelFileService,
        private translate: TranslateService,
        private asFeatureService: AsFeatureService, private storeCurrentUserService: StoreCurrentUserService) {
    ...
    public onExportExcel(): void {
        this.listSimCard$.subscribe(data => {
            if (data) { this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "list_sim_cards"); }
        });
    }
    ...
    getStatusSimCardBadge(selectedSimCard?: { statut: T_SIM_CARD_STATUS_ENUM }): TYPE_COLOR_STATUS_BADGE {
        if (!selectedSimCard || !selectedSimCard.statut) { return 'badge-dark'; }

        const etapeMap: Record<T_SIM_CARD_STATUS_ENUM, TYPE_COLOR_STATUS_BADGE> = {
    ...
    getIdentificationSimCardBadge(selectedSimCard?: { identification_fiabilite: T_SIM_CARD_IDENTIFICATION_ENUM }): TYPE_COLOR_IDENTIFICATION_BADGE {
        if (!selectedSimCard || !selectedSimCard.identification_fiabilite) { return 'badge-success'; }

        const etapeMap: Record<T_SIM_CARD_IDENTIFICATION_ENUM, TYPE_COLOR_IDENTIFICATION_BADGE> = {
        ...
        return etapeMap[selectedSimCard.identification_fiabilite] || 'badge-success';
    }
    ...
    public handleAction(params: Action): void {
        ...
        switch (params.view) {
            case 'modal':
                if (params.action === 'map-sim-card') { this.handleSimCardMap(params.data) }
                if (params.action === 'qr-code-sim-card') { this.handleSimCardQrCode(params.data) }
                break;

            case 'page':
                if (params.action === 'view-sim-card') { this.interfaceUser.emit(params) };
                if (params.action === 'update-sim-card') { this.interfaceUser.emit(params) };
                if (params.action === 'identification-sim-card') { this.interfaceUser.emit(params) };
                break;
        }
    }

    // Méthode pour gérer l'affichage de la carte
    private async handleSimCardMap(selectedSimCard: simCardInterface): Promise<void> {
    ...
    private getGpsCoordinates(selectedSimCard: any): [number, number] | null {
        const longitude = parseFloat(selectedSimCard.longitude ?? selectedSimCard.long_reseau ?? '');
        console.log('longitude', longitude)
        const latitude = parseFloat(selectedSimCard.latitude ?? selectedSimCard.lat_reseau ?? '');
        console.log('latitude', latitude)
        if (isNaN(longitude) || isNaN(latitude)) { return null; }
        return [longitude, latitude];
    }
    private initializeMap(coords: [number, number], selectedSimCard: simCardInterface): void {
        console.log('coords', coords)
        setTimeout(() => {
            const mapContainer = document.getElementById("containerSimCardMap");
            if (!mapContainer) {
                console.error("Élément HTMLElement id='containerSimCardMap' introuvable !");
                return;
            }
            ...
            if (this.map) {
                const traficMarker = this.createMarker(selectedSimCard, coords, this.traficIcon, 'trafic');
                const networkMarker = this.createMarker(selectedSimCard, coords, this.networkIcon, 'reseau');
                ...
                console.error("Impossible d'ajouter des marqueurs : la carte Leaflet est undefined !");
            }
        }, 300);
    }
    ...
    private createMarker(
        ...
        const coordinates: L.LatLngExpression = type === 'trafic'
            ? [parseFloat(simCard.longitude ?? simCard.long_reseau), parseFloat(simCard.latitude ?? simCard.lat_reseau)]
            : [parseFloat(simCard.long_reseau ?? simCard.longitude), parseFloat(simCard.lat_reseau ?? simCard.latitude)];
    ...
    private generatePopupContent(simCard: any, type: 'trafic' | 'reseau'): string {
        return `
            <div>
                <strong>Numéro SIM :</strong> <span>${simCard.msisdn ?? ''}</span><br>
                <strong>${this.firstLevelLibel} :</strong> <span>${simCard.niveau_uns_nom ?? ''}</span><br>
                <strong>${this.secondLevelLibel} :</strong> <span>${simCard.niveau_deux_nom ?? ''}</span><br>
                <strong>Type d'emplacement :</strong> <span>${simCard.niveau_trois_nom ?? ''}</span><br>
                <strong>Nom Emplacement :</strong> <span>${simCard.point_emplacement ?? ''}</span><br>
                ${type === 'reseau' ? `
                    <strong>Site :</strong> <span>${simCard.site_reseau ?? ''}</span><br>
                    <strong>Geoloc :</strong> <span>${simCard.geoloc ?? ''}</span><br>
                    <strong>Quartier :</strong> <span>${simCard.quartier ?? ''}</span><br>
                    <strong>Date Trafic :</strong> <span>${simCard.date_localisation ?? ''}</span><br>
                ` : ''}
                <strong>Statut :</strong> <span>${simCard.statut ?? ''}</span><br>
                <strong>Coordonnées GPS :</strong> <span>${type === 'trafic'
                ? `${simCard.longitude ?? ''}, ${simCard.latitude ?? ''}`
                : `${simCard.long_reseau ?? ''}, ${simCard.lat_reseau ?? ''}`}
                </span><br>
            </div>
        `;
    }
    ...
    public handleSimCardQrCode(selectedSimCard: simCardInterface): void {
        if (selectedSimCard) {
            const qrCodeModalRef = this.ngbModal.open(QrModalComponent, { ...ModalParams, backdrop: true, keyboard: true });
    ...
    public getTreatmentButtonViewSimCardStyle(selectedSimCard: { msisdn: string }): { tooltip: string } {
        const DETAILS_OF_THE_SIM = this.translate.instant('DETAILS_OF_THE_SIM').replace('{MSISDN}', selectedSimCard.msisdn);
        return { tooltip: DETAILS_OF_THE_SIM };
    }

    public getTreatmentButtonMapSimCardStyle(selectedSimCard: ButtonMapSimCardData): ButtonMapSimCardStyle {
        const LOCATION_OF_THE_SIM = this.translate.instant('LOCATION_OF_THE_SIM');
        const CANNOT_SEE_THE_LOCATION = this.translate.instant('CANNOT_SEE_THE_LOCATION');
        if ((!selectedSimCard?.latitude && !selectedSimCard?.lat_reseau) && !selectedSimCard?.date_localisation) {
            return { disabled: true, tooltip: CANNOT_SEE_THE_LOCATION };
        }
        return { disabled: false, tooltip: LOCATION_OF_THE_SIM.replace('{MSISDN}', selectedSimCard.msisdn) };
    }

    public getTreatmentButtonQrCodeSimCardStyle(selectedSimCard: ButtonQrCodeSimCardData): ButtonQrCodeSimCardStyle {
        const QR_CODE_OF_THE_SIM = this.translate.instant('QR_CODE_OF_THE_SIM');
        const CANNOT_SEE_THE_QR_CODE = this.translate.instant('CANNOT_SEE_THE_QR_CODE')
        if (!selectedSimCard?.qrcode) {
            return { disabled: true, tooltip: CANNOT_SEE_THE_QR_CODE };
        }
        return { disabled: false, tooltip: QR_CODE_OF_THE_SIM.replace('{MSISDN}', selectedSimCard.msisdn) };
    }
}
