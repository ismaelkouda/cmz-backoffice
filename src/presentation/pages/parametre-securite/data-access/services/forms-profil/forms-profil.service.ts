...
export class FormsProfilApiService {
    ...
    fetchPermissions(profilId: string): void {
        ...
        const url: string = formsProfilEndpointEnum.USER_PROFILES_ID.replace('{/id}', profilId);
            ...
            .pipe(
                ...
                switchMap((response: any) => {
                    const permissions = response?.['data']?.permissions.map((permission) => {
                        ...
                        return {
                            ...permission['data'], label: permission['data']?.title,
                            children: permission?.['children']?.map((child) => {
                                return { ...child['data'], label: child['data']?.title };
                            })
                        };
                    });
    ...
    private apiResponseHabilitationSubject = new BehaviorSubject<any>({} as any);
    fetchHabilitation(profilId: string): void {
        ...
        this.http
            ...
            .pipe(
                debounceTime(500),
                switchMap((response: any) => {
                    const habilitation = response?.['data']?.habilitations.map((habilitation) => {
                        console.log('habilitation', habilitation)
                        return {
                            ...habilitation['data'], label: habilitation['data']?.title,
                            children: habilitation?.['children']?.map((child) => {
                                return { parent_value:  habilitation['data'].value, ...child['data'], label: child['data']?.title };
                            })
                        };
                    });
