...
export class DemandesProduitsService {
    ...
    postCommandeProduitCommandesAll(data, page): Observable<any> {
      const url: string = <string>EndPointUrl.POST_COMMANDE_PRODUIT_COMMANDES_ALL.replace(
        "{page}",
        page
      );
    ...
    postCommandeProduitCommandesStore(data): Observable<any> {
      const url: string = <string>EndPointUrl.POST_COMMANDE_PRODUIT_COMMANDES_STORE;
      return this.http.post(`${this.baseUrl}${url}`, data);
    }
    postCommandeProduitTransactionAll(data, page): Observable<any> {
      const url: string = (<string>EndPointUrl.POST_COMMANDE_PRODUIT_TRANSACTION_ALL).replace(
        "{page}",
        page
      );
      return this.http.post(`${this.baseUrl}${url}`, data);
