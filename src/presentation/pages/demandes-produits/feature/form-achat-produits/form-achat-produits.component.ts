...
export class FormAchatProduitsComponent implements OnInit {
    ...
    public fileModel =
        'src/assets/data/Modele-Chargement-Numéros-En-Masse.xlsx';
    ...
    public GetCoutUnitaireOperation(): void {
        this.settingService
            .GetCoutUnitaireOperation(OperationTransaction.SIM_BLANCHE)
            .subscribe({
                next: (response) => {
    ...
    async pageCallback(
        ...
        const response: any = await handle(
            () =>
                this.demandesProduitsService.postCommandeProduitCommandesAll(
                    urlParamFilter,
                    urlParamCurrentPage
                ),
            this.toastrService,
        ...
        if (response.error === false)
            this.handleSuccessfulPageCallback(response, urlParamFilter);
    }
    ...
    private getAchatSelected(listAchatProduits: Array<Object>): void {
        this.achatSelected = listAchatProduits.find(
            (achat) => achat?.['numero_demande'] == this.urlParamId
        );
    ...
    private initAchatProduitsForm(): void {
        this.achatForm = this.fb.group({
            ...
            nb_demandes: this.createFormControl(
                this.achatSelected?.['nb_demandes'],
                Validators.required,
                false
            ),
            prix_unitaire: this.createFormControl(
                this.achatSelected?.['prix_unitaire'],
                Validators.required,
                true
            ),
            description: this.createFormControl(
                this.achatSelected?.['description'],
                Validators.required,
                false
            ),
            montant: this.createFormControl(
                this.achatSelected?.['montant'],
                Validators.required,
                true
            ),
            justificatif: this.createFormControl(
                null,
                Validators.required,
                false
            ),
    ...
    public pushCurrentArrayForm(file_upload: any) {
        if (file_upload)
            this.achatForm.get('sims_file')?.patchValue(file_upload.sims_file);
    }
        ...
        switch (this.urlParamRef) {
            case 'ajouter':
                if (this.achatForm.valid) {
                    const response: any = await handle(
                        () =>
                            this.demandesProduitsService.postCommandeProduitCommandesStore(
                                FormatFormData(this.achatForm.value)
                            ),
                        this.toastrService,
                        this.loadingBarService
                    );
                    if (response?.error === false)
                        this.handleSuccessfulAchatProduitsForm(response);
                }
                break;
