...
import { QUEUES_TABLE } from '@presentation/pages/processing/presentation/adapters/queues/queues-table.constant';
...
export class QueuesComponent {
    ...
    private readonly exportTooltip = computed(() => {
        ...
        if (permission) {
            return this.t('PROCESSING.QUEUES.TOOLTIP.NO_PERMISSION_EXPORT');
        }
        if (noData) {
            return this.t('PROCESSING.QUEUES.TOOLTIP.NO_EXPORT');
        }
        return this.t('PROCESSING.QUEUES.TOOLTIP.EXPORT').replace(
            '{nb}',
    ...
    private exportData(): void {
        ...
        this.excelExport
            .exportToExcel({
                ...
                data: items,
                sheetName: this.translate.instant('PROCESSING.QUEUES.TITLE'),
                autoFilter: true,
            })
            .catch((err) => {
