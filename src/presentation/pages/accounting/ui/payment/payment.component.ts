...
import { paymentApiResponseInterface, paymentGlobalStatsInterface, paymentInterface } from '../../data-access/payment/interface/payment.interface';
...
type PageAction = { 'data': paymentInterface, 'action': 'view-payment', 'view': 'page' };
...
export class PaymentComponent implements OnInit, OnDestroy {
    ...
    constructor(private activatedRoute: ActivatedRoute, private paymentApiService: PaymentApiService,
    ...
    ngOnInit(): void {
        ...
        this.listPaymentResponse$ = this.paymentApiService.getApiResponsePayment();
        this.pagination$ = this.paymentApiService.getPaymentPagination();
        this.paymentApiService.getPaymentGlobalState().pipe(takeUntil(this.destroy$)).subscribe((globalState) => {
        ...
        this.paymentApiService.isLoadingPayment().pipe(takeUntil(this.destroy$)).subscribe((spinner: boolean) => {
    ...
    public onPageChange(event: number): void {
        this.paymentApiService.getDataFilterPayment().pipe(takeUntil(this.destroy$)).subscribe((filterData) => {
            this.paymentApiService.fetchPayment(filterData, JSON.stringify(event + 1))
        });
    }
    ...
    public navigateByUrl(params: PageAction): void {
        ...
        switch (params.action) {
            case "view-payment": routePath = `${number_demand}`; this.router.navigate([routePath], { relativeTo: this.activatedRoute, queryParams }); break;
        }
    }

    private getStatisticsBoxValues(rapport: paymentGlobalStatsInterface | {} = {}): void {
        this.statisticsBox = [
            {
                id: 0,
    ...
    public onBoxClick(statistiqueBox: IStatistiquesBox) {
        ...
        if (indexBoxClickable.includes(statistiqueBox.id as IndexBoxClickable)) {
            switch (statistiqueBox.id) {
                case 1: this.paymentApiService.fetchPayment({ statut: "postée" } as any); break;
                case 2: this.paymentApiService.fetchPayment({ statut: "reportée" } as any); break;
                case 3: this.paymentApiService.fetchPayment({ statut: "soldée" } as any); break;
                case 4: this.paymentApiService.fetchPayment({ statut: "rejetée" } as any); break;
            }
        }
    }
