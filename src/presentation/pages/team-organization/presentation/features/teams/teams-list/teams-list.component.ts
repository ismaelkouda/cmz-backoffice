...
import { TEAMS_TABLE } from '@presentation/pages/team-organization/presentation/adapters/teams/teams-table.constant';
import { TeamsVmProps } from '@presentation/pages/team-organization/presentation/adapters/teams/teams-vm-props.interface';
...
export class TeamsListComponent {
    ...
    private readonly formStore = inject(TeamsFilterStore);
    ...
    private readonly exportTooltip = computed(() => {
        ...
        if (permission) {
            return this.t(
                'TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_PERMISSION_EXPORT'
            );
        }
        if (noData) {
            return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_EXPORT');
        }
        return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.EXPORT').replace(
            '{nb}',
    ...
    private readonly deleteTooltip = computed(() => {
        if (!this.canDelete()) {
            return this.t(
                'TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_PERMISSION_DELETE'
            );
        }
        return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.NOT_DELETE');
    });
    private readonly enableTooltip = computed(() => {
        if (!this.canEnable()) {
            return this.t(
                'TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_PERMISSION_ACTIVE'
            );
        }
        return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.NOT_ACTIVE');
    });
    private readonly disableTooltip = computed(() => {
        if (!this.canDisable()) {
            return this.t(
                'TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_PERMISSION_DISABLE'
            );
        }
        return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.NOT_DISABLE');
    });
    private readonly chooseTooltip = computed(() => {
        if (!this.canChoose()) {
            return this.t(
                'TEAM_ORGANIZATION.TEAMS.TOOLTIP.NO_PERMISSION_CHOOSE'
            );
        }
        return this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.NOT_CHOOSE');
    });
    protected readonly headerButtons = computed<TableHeaderButton[]>(() => [
        ...
        },
        {
            ...
            tooltip: this.t('TEAM_ORGANIZATION.TEAMS.TOOLTIP.REFRESH'),
        },
        {
            label: 'COMMON.EXPORT',
            actionId: 'export',
    ...
    constructor() {
        this.facade.readAll(this.currentFilter() as TeamsFilterDto);
    ...
    private pageTitle(): void {
        this.currentLang();
        this.title.setTitle(this.t('TEAM_ORGANIZATION.TEAMS.TITLE'));
    }
    ...
    private exportData(): void {
        ...
        //     items,
        //     this.tableConfig,
        //     `${this.normalizeExportPrefix}-teams`
        // );
    }
