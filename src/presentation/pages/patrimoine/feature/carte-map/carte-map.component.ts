...
export class CarteMapComponent implements AfterViewInit {
    ...
    ngAfterViewInit(): void {
        this.parcelleMap.nativeElement.innerHTML =
            "<div id='map' style='height: 45vw;width: 100% !important'></div>";
        setTimeout(() => {
            this.onMapReady();
        }, 1000);
    }
    public onMapReady() {
        ...
        this.map.setView(
            new L.LatLng(this.patrimoine?.longitude, this.patrimoine?.latitude),
            9
        );
        this.map.options.minZoom = 12;
        var marker = L.marker([
            this.patrimoine?.longitude,
            this.patrimoine?.latitude,
        ])
            .setIcon(customIcon)
            .bindPopup(
                '<div>' +
                    ...
                    ' :</strong>' +
                    '<span>' +
                    this.patrimoine?.direction_regionale?.nom +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    this.secondLevelLibelle +
                    ' :</strong>' +
                    '<span>' +
                    this.patrimoine?.exploitation?.nom +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    this.thirdLevelLibelle +
                    ' :</strong>' +
                    '<span>' +
                    this.patrimoine?.zone?.nom +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    'Nom Emplacement :' +
                    '</strong>' +
                    '<span>' +
                    this.patrimoine?.nom_prenoms +
                    '</span>' +
                    '<br>' +
                    '<strong>Statut :</strong>' +
                    '<span>' +
                    this.patrimoine?.statut +
                    '</span>' +
                    '<br>' +
                    '</div>'
            )
