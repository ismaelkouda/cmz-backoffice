...
import { SIM_CARD_IDENTIFICATION_ENUM, T_SIM_CARD_IDENTIFICATION_ENUM } from '../../../../patrimony/data-access/sim-card/enums/sim-card-identification.enum';
import { SIM_CARD_STATUS_ENUM, T_SIM_CARD_STATUS_ENUM } from '../../../../patrimony/data-access/sim-card/enums/sim-card-status.enum';
import { AsFeatureService } from '../../../../../../shared/services/as-feature.service';

type Action = PageAction | ModalAction;
type PageAction = { data: Object, action: 'view-sim-card' | 'update-sim-card' | 'identification-sim-card', view: 'page' };
type ModalAction = { data: Object, action: 'map-sim-card' | 'qr-code-sim-card', view: 'modal' };
type ButtonMapSimCardStyle = { disabled: boolean, tooltip: string };
type ButtonMapSimCardData = { latitude: string, longitude: string, date_localisation: string, msisdn: string };
...
export class CarteSimTableComponent {
  ...
  constructor(public toastService: ToastrService, private clipboardService: ClipboardService,
    private ngbModal: NgbModal, private asFeatureService: AsFeatureService,
    private mappingService: MappingService, private patrimoinesService: PatrimoinesService,
  ...
  public onExportExcel(): void {
    this.listSimCard$.subscribe(data => {
      if (data) {
        this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "list_sim_cards");
      }
    });
  }
  ...
  getIdentificationSimCardBadge(selectedSimCard?: any): TYPE_COLOR_IDENTIFICATION_BADGE {
  ...
  private createMarker(
    ...
    const coordinates: L.LatLngExpression = type === 'trafic'
      ? [parseFloat(simCard.longitude ?? ''), parseFloat(simCard.latitude ?? '')]
      : [parseFloat(simCard.long_reseau ?? ''), parseFloat(simCard.lat_reseau ?? '')];
    const marker = L.marker(coordinates, { icon });
    const popupContent = this.generatePopupContent(simCard, type);
    marker.bindPopup(popupContent).openPopup();
    return marker;
  }
  private initializeMap(coords: [number, number], selectedSimCard: any): void {
    setTimeout(() => {
      ...
      if (this.map) {
        const traficMarker = this.createMarker(selectedSimCard, this.traficIcon, 'trafic');
        const networkMarker = this.createMarker(selectedSimCard, this.networkIcon, 'reseau');
        ...
        L.control.layers(baseMaps, layerGeoJson, { collapsed: true }).addTo(this.map);
      } else {
  ...
  handleSimCardQrCode(selectedSimCard: any): void {
    if (selectedSimCard) {
      const qrCodeModalRef = this.ngbModal.open(QrModalComponent, { ...ModalParams, size: 'lg', backdrop: true, keyboard: true });
  ...
  getTreatmentButtonViewSimCardStyle(selectedSimCard: { msisdn: string }): { tooltip: string } {
  ...
  getTreatmentButtonMapSimCardStyle(selectedSimCard: ButtonMapSimCardData): ButtonMapSimCardStyle {
    ...
    if ((!selectedSimCard?.latitude && !selectedSimCard?.longitude) && !selectedSimCard?.date_localisation) {
  ...
  getTreatmentButtonQrCodeSimCardStyle(selectedSimCard: ButtonQrCodeSimCardData): ButtonQrCodeSimCardStyle {
    ...
    return { disabled: true, tooltip: QR_CODE_OF_THE_SIM.replace('{MSISDN}', selectedSimCard.msisdn) };
  }
}