...
import {
    WhiteSimCardDetails,
    WhiteSimCardDetailsResponse,
} from './data-access/white-sim-card-details/table-white-sim-card-details';
import { PatrimoinesService } from '../../../data-access/patrimoines.service';
import {
    DossierWhiteSimCard,
    WhiteSimCardPaginatedResponse,
...
export class WhiteSimCardDetailsComponent implements OnInit {
    ...
    constructor(
        ...
        private stateWhiteSimCardService: StateWhiteSimCardService,
    ...
    private getParamsInUrl(): void {
        this.activatedRoute.queryParams.subscribe((params: Object) => {
            ...
            this.urlParamFilter =
                this.stateWhiteSimCardService.getFilterWhiteSimCardState(
                    params?.['filter']
                );
        });
        ...
        this.filterData = {
            imsi: this.urlParamFilter?.['imsi'],
            iccid: this.urlParamFilter?.['iccid'],
        };
    }

    async pageCallback(
        urlParamFilter: Object = {},
        urlParamCurrentPage: string = '1'
    ) {
        const response: WhiteSimCardPaginatedResponse | void =
            await handle<WhiteSimCardPaginatedResponse>(
                () =>
                    this.patrimoinesService.PostPatrimoineSimCartonSimBlancheAllPage(
                        urlParamFilter,
                        urlParamCurrentPage
                    ),
                this.toastrService,
    ...
    private getWhiteSimCardSelected(
        listWhiteSimCard: Array<DossierWhiteSimCard>
    ): void {
        this.selectedWhiteSimCard = listWhiteSimCard.find(
            (carteSim: DossierWhiteSimCard) =>
                carteSim?.['id'] == this.urlParamId
        );
        if (this.selectedWhiteSimCard) {
            this.sharedDataService
                .postPatrimoineDetailsWhiteSimCard()
                .subscribe(() => {
                    this.postPatrimoineDetailsWhiteSimCard({
                        id: this.selectedWhiteSimCard?.['id'],
                    });
                });
            this.postPatrimoineDetailsWhiteSimCard({
                id: this.selectedWhiteSimCard?.['id'],
    ...
    async postPatrimoineDetailsWhiteSimCard(
        dataToSend: Object = this.filterData,
        nbrPage: number = 1
    ) {
        const response: WhiteSimCardDetailsResponse | any =
            await handle<WhiteSimCardDetailsResponse>(
                () =>
                    this.patrimoinesService.PostPatrimoineSimCartonSimBlancheDetailsPage(
                        {
                            ...dataToSend,
                            id: this.selectedWhiteSimCard?.['id'],
                        },
        ...
        if (response && response.error === false)
            this.handleSuccessfulPostPatrimoineDetailsWhiteSimCard(response);
    }

    private handleSuccessfulPostPatrimoineDetailsWhiteSimCard(
        response: any
    ): void {
        this.listWhiteSimCardDetails = response.data;
        this.spinner = false;
        this.stateWhiteSimCardService.setCurrentPageWhiteSimCardState(
            this.urlParamCurrentPage
        );
        this.stateWhiteSimCardService.setFilterWhiteSimCardState(
            this.urlParamFilter
        );
        this.stateWhiteSimCardService.setItemSelectedState(
            this.selectedWhiteSimCard
        );
    }
