...
import {
    DEMANDE_FORMULE_CHANGE,
    DEMANDE_RESILIATION,
    DEMANDE_SWAPPING,
} from './../../../demandes/demandes-routing.module';
...
import {
    DEMANDE_ACTIVATION,
    DEMANDE_SUSPENSION,
} from 'src/presentation/pages/demandes/demandes-routing.module';
...
export class CarteSimActiveComponent implements OnInit {
    ...
    public title =
        'Carte SIM actives - Système de Gestion de Collecte Centralisée';
    constructor(
    ...
    async pageCallback(dataToSend = {}, nbrPage: number = 1) {
        this.response = await handle(
            () => this.patrimoineService.GetAllPatrimoines(dataToSend, nbrPage),
            this.toastrService,
            this.loadingBar
        );
    ...
    async OnChangeStatut() {
        ...
        this.response = await handle(
            () => this.patrimoineService.OnChangeStatut(dataToSend),
            this.toastrService,
            this.loadingBar
        );
    ...
    public onDotationForm(data: any): void {
        this.onMarkItemCarteSim(data);
        this.initialView = false;
        this.router.navigateByUrl(`${PATRIMOINE}/${DOTATION_SERVICES}`, {
    ...
    hasLocation(data: any): boolean {
        return (
            (data.longitude && data.latitude) ||
            (data.long_reseau && data.lat_reseau)
        );
    }
    ...
    public onMapReady() {
        ...
        this.map.setView(
            new L.LatLng(
                this.currentComposant?.longitude ??
                    this.currentComposant?.long_reseau,
                this.currentComposant?.latitude ??
                    this.currentComposant?.lat_reseau
            ),
            18
        );
        ...
        const googlemap = new L.TileLayer(
        ...
        var traficPoint = L.marker([
            ...
            .bindPopup(
                '<div>' +
                    ...
                    '<span>' +
                    this.currentComposant?.msisdn +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    this.firstLevelLibelle +
                    ' :</strong>' +
                    '<span>' +
                    this.currentComposant?.niveau_uns_nom +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    this.secondLevelLibelle +
                    ' :</strong>' +
                    '<span>' +
                    this.currentComposant?.niveau_deux_nom +
                    '</span>' +
                    '<br>' +
                    ...
                    '</strong>' +
                    '<span>' +
                    `${this.currentComposant?.niveau_trois_nom ?? ''}` +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    'Nom Emplacement :' +
                    '</strong>' +
                    '<span>' +
                    `${this.currentComposant?.point_emplacement ?? ''}` +
                    '</span>' +
                    '<br>' +
                    '<strong>Statut :</strong>' +
                    '<span>' +
                    `${this.currentComposant?.statut ?? ''}` +
                    '</span>' +
                    '<br>' +
        ...
        var reseauPoint = L.marker([
            ...
            .bindPopup(
                '<div>' +
                    ...
                    //"<strong>" + this.thirdLevelLibelle + " :</strong>" + "<span>" + this.currentComposant?.niveau_trois_nom + "</span>" + "<br>" +
                    '<strong>' +
                    "Type d'emplacement :</strong>" +
                    '<span>' +
                    `${this.currentComposant?.niveau_trois_nom ?? ''}` +
                    '</span>' +
                    '<br>' +
                    '<strong>' +
                    'Nom Emplacement :</strong>' +
                    '<span>' +
                    `${this.currentComposant?.point_emplacement ?? ''}` +
                    '</span>' +
                    '<br>' +
                    ...
                    (this.currentComposant?.site_reseau ?? '') +
                    '</span><br>' +
                    '<strong>Geoloc :</strong> <span>' +
                    (this.currentComposant?.geoloc ?? '') +
                    '</span><br>' +
                    '<strong>Quartier :</strong>' +
                    '<span>' +
                    `${this.currentComposant?.quartier ?? ''}` +
                    '</span>' +
                    '<br>' +
                    '<strong>Statut :</strong>' +
                    '<span>' +
                    ...
                    '</span>' +
                    '<br>' +
                    '<strong>Date Trafic :</strong>' +
                    '<span>' +
                    `${this.currentComposant?.date_localisation ?? ''}` +
                    '</span>' +
                    '<br>' +
                    '<strong>Coordonnées GPS :</strong>' +
                    '<span>' +
        ...
        var layerGeoJson = {
            "<span style='font-weight:bold;'><b>Position déclarée</b></span><span><img src='assets/svg/sim_loc_noir.svg' style='width: 10px; margin-left: 20px;'/></span>":
                traficPoint,
            "<span style='font-weight:bold'><b>Position trafic</b></span><span><img src='assets/svg/sim_loc_orange.svg' style='width: 10px; margin-left: 20px;'/></span>":
                reseauPoint,
        };
        L.control
    ...
    public disableAction(): boolean {
        return this.listPatrimoines === undefined ||
            this.listPatrimoines?.length === 0
            ? true
            : false;
    }
}
