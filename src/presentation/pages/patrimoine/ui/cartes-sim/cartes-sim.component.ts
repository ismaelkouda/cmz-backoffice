...
export class CartesSimComponent implements OnInit {
  ...
  constructor(public toastrService: ToastrService, private router: Router,
    private patrimoinesService: PatrimoinesService, private activatedRoute: ActivatedRoute, 
  ...
  public pageCallback(): void {
    ...
    combineLatest([
      ...
      this.patrimoinesService.fetchSims({ statut: history?.state?.statut, filterData }, nbrPageData);
    });
  }
  ...
  public onPageChange(event: number) {
    this.patrimoinesService.fetchSims(this.filterData, JSON.stringify(event + 1))
  }
