...
export class PatrimoinesService {
  ...
  fetchSims(data: Object, nbrPage: string = '1'): void {
    ...
    const url: string = EndPointUrl.POST_PATRIMOINE_SIM_SIMS_ALL_PAGE.replace('{page}', nbrPage)
    ...
    this.httpClient
      ...
      .pipe(
        ...
        switchMap((response: any) => {
          this.simsSubject.next(response?.['data']?.data.map((demande) => {
  ...
  fetchSimsDetails(imsi: string): void {
    ...
    const url: string = EndPointUrl.POST_PATRIMOINE_SIM_SIMS_imsi_Details.replace('{imsi}', imsi)
  ...
  GetAllSmsBalanceStatus(data, page): Observable<any> {
    const url: string = (<string>EndPointUrl.GET_ALL_ETAT_SOLDE_SMS).replace('{page}', page);
    return this.httpClient.post(`${this.baseUrl}${url}`, data);
  }
