...
import { ApprovePresenter } from '@pages/report-states/presentation/adapters/approve/approve-vm.presenter';
import { ApproveFilterStore } from '@pages/report-states/presentation/store/approve/approve-filter.store';
import { APPROVE_TABLE } from '@presentation/pages/report-states/presentation/adapters/approve/approve-table.constants';
...
export class ApproveComponent {
    ...
    protected readonly facade = inject(ApproveFacade);
    private readonly translate = inject(TranslateService);
    private readonly toast = inject(ToastrService);
    private readonly formStore = inject(ApproveFilterStore);
    ...
    protected readonly tableConfig = APPROVE_TABLE;
        ...
        return [
            {
                ...
                label: this.t('REPORT_STATES.APPROVE.FILTER.INITIATOR'),
                placeholder: this.t('COMMON.PHONE_PLACEHOLDER'),
                icon: 'pi pi-phone',
                translationKeys: {
                    label: 'REPORT_STATES.APPROVE.FILTER.INITIATOR',
            ...
            },
            {
                type: 'select',
                name: 'reportType',
                label: this.t('REPORT_STATES.APPROVE.FILTER.REPORT_TYPE'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.APPROVE.FILTER.REPORT_TYPE',
                },
                class: 'p-long',
            },
            {
                type: 'multi-select',
                name: 'operators',
                label: this.t('REPORT_STATES.APPROVE.FILTER.OPERATORS'),
                placeholder: this.t('COMMON.SELECT_PLACEHOLDER'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.APPROVE.FILTER.OPERATORS',
                },
                class: 'p-medium',
            },
            {
                type: 'select',
                name: 'source',
                label: this.t('REPORT_STATES.APPROVE.FILTER.SOURCE'),
                placeholder: this.t('COMMON.SELECT_PLACEHOLDER'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.APPROVE.FILTER.SOURCE',
                },
            },
            {
                type: 'date',
    ...
    protected readonly headerButtons = computed<TableHeaderButton[]>(() => [
        ...
        },
        {
            ...
            tooltip: this.t('REPORT_STATES.APPROVE.TOOLTIP.REFRESH'),
        },
        {
            label: 'COMMON.EXPORT',
            actionId: 'export',
    ...
    private readonly downloadModalEffect = effect(() => {
    ...
    protected readonly downloadTooltip = computed(() => {
        ...
        if (noFilter) {
            return this.t('REPORT_STATES.APPROVE.TOOLTIP.NO_FILTER');
        }
        if (noData) {
            return this.t('REPORT_STATES.APPROVE.TOOLTIP.NO_DOWNLOAD');
        }
        return this.t('REPORT_STATES.APPROVE.TOOLTIP.DOWNLOAD').replace(
            '{nb}',
    ...
    protected async onDownloadClicked(): Promise<void> {
        ...
        const confirmed = await this.sweetAlert.confirm({
            titleKey: 'REPORT_STATES.APPROVE.SWEET_ALERT.TITLE.DOWNLOAD',
            messageKey: 'REPORT_STATES.APPROVE.SWEET_ALERT.MESSAGE.DOWNLOAD',
            messageParams: {
    ...
    private readonly presenter = new ApprovePresenter(
    ...
    private readonly exportTooltip = computed(() => {
        ...
        const noData = this.itemsVM().length < 1;
        if (permission) {
            return this.t('REPORT_STATES.APPROVE.TOOLTIP.NO_PERMISSION_EXPORT');
        }
        if (noData) {
            return this.t('REPORT_STATES.APPROVE.TOOLTIP.NO_EXPORT');
        }
        return this.t('REPORT_STATES.APPROVE.TOOLTIP.EXPORT').replace(
            '{nb}',
            String(this.itemsVM().length)
        );
    });
    constructor() {
        this.facade.read(this.currentFilter() as ApproveFilterDto);
    ...
    private pageTitle(): void {
        this.currentLang();
        this.title.setTitle(this.t('REPORT_STATES.APPROVE.TITLE'));
    }
    ...
    private exportData(): void {
        ...
        const fileName = `${this.exportFilePrefix}-approve`;

        const exportColumns: ExportColumn[] = APPROVE_TABLE.cols
