...
import { EvaluatePresenter } from '@pages/report-states/presentation/adapters/evaluate/evaluate-vm.presenter';
import { EvaluateFilterStore } from '@pages/report-states/presentation/store/evaluate/evaluate-filter.store';
import { EVALUATE_TABLE } from '@presentation/pages/report-states/presentation/adapters/evaluate/evaluate-table.constant';
...
export class EvaluateComponent {
    ...
    protected readonly facade = inject(EvaluateFacade);
        ...
        return [
            {
                ...
                label: this.t('REPORT_STATES.EVALUATE.FILTER.INITIATOR'),
                placeholder: this.t('COMMON.PHONE_PLACEHOLDER'),
                icon: 'pi pi-phone',
                translationKeys: {
                    label: 'REPORT_STATES.EVALUATE.FILTER.INITIATOR',
            ...
            },
            {
                type: 'select',
                name: 'reportType',
                label: this.t('REPORT_STATES.EVALUATE.FILTER.REPORT_TYPE'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.EVALUATE.FILTER.REPORT_TYPE',
                },
                class: 'p-long',
            },
            {
                type: 'multi-select',
                name: 'operators',
                label: this.t('REPORT_STATES.EVALUATE.FILTER.OPERATORS'),
                placeholder: this.t('COMMON.SELECT_PLACEHOLDER'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.EVALUATE.FILTER.OPERATORS',
                },
                class: 'p-medium',
            },
            {
                type: 'select',
                name: 'source',
                label: this.t('REPORT_STATES.EVALUATE.FILTER.SOURCE'),
                placeholder: this.t('COMMON.SELECT_PLACEHOLDER'),
                ...
                icon: 'pi pi-filter',
                translationKeys: {
                    label: 'REPORT_STATES.EVALUATE.FILTER.SOURCE',
                },
            },
            {
                type: 'date',
    ...
    protected readonly headerButtons = computed<TableHeaderButton[]>(() => [
        ...
        },
        {
            ...
            tooltip: this.t('REPORT_STATES.EVALUATE.TOOLTIP.REFRESH'),
        },
        {
            label: 'COMMON.EXPORT',
            actionId: 'export',
    ...
    private readonly exportTooltip = computed(() => {
        ...
        if (permission) {
            return this.t(
                'REPORT_STATES.EVALUATE.TOOLTIP.NO_PERMISSION_EXPORT'
            );
        }
        if (noData) {
            return this.t('REPORT_STATES.EVALUATE.TOOLTIP.NO_EXPORT');
        }
        return this.t('REPORT_STATES.EVALUATE.TOOLTIP.EXPORT').replace(
            '{nb}',
            String(this.itemsVM().length)
        );
    });
    constructor() {
        this.facade.read(this.currentFilter() as EvaluateFilterDto);
    ...
    private exportData(): void {
        ...
        const fileName = `${this.exportFilePrefix}-evaluate`;

        const exportColumns: ExportColumn[] = EVALUATE_TABLE.cols
