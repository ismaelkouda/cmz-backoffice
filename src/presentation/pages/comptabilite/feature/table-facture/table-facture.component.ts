...
type PageAction =
    ...
    | { data: Object; action: 'facture'; view: 'page' };
...
export class TableFactureComponent {
    ...
    public getColorActionButton(selectedFacture: Object): Object {
        if (!!selectedFacture?.['type_paiement']) {
            return {
                style: 'badge-success',
                value: selectedFacture?.['etat_facture'],
            };
        } else {
            return {
                style: 'badge-danger',
    ...
    public isDisabledActionButton(): boolean {
        return (
            this.startedDay !== 'démarré' && this.userRole === 'gestionnaire'
        );
    }
}
