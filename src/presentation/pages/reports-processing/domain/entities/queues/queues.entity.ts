...
export class QueuesEntity implements Queues {
    ...
    public get reportDescription(): string {
        const reportDescription: Record<string, string> = {
            ABI: 'REPORTS_PROCESSING.QUEUES.OPTIONS.REPORT_TYPE.NO_INTERNET',
            CPS: 'REPORTS_PROCESSING.QUEUES.OPTIONS.REPORT_TYPE.PARTIAL_SIGNAL',
            CPO: 'REPORTS_PROCESSING.QUEUES.OPTIONS.REPORT_TYPE.PARTIAL_OPERATOR',
            ZOB: 'REPORTS_PROCESSING.QUEUES.OPTIONS.REPORT_TYPE.WHITE_ZONE',
        };
    ...
    public get sourceDescription(): string {
        const sourceDescription: Record<string, string> = {
            APP: 'REPORTS_PROCESSING.QUEUES.OPTIONS.SOURCE.APP',
            USSD: 'REPORTS_PROCESSING.QUEUES.OPTIONS.SOURCE.USSD',
            SMS: 'REPORTS_PROCESSING.QUEUES.OPTIONS.SOURCE.SMS',
            IVR: 'REPORTS_PROCESSING.QUEUES.OPTIONS.SOURCE.IVR',
        };
    ...
    public toString(): string {
        return `QueuesQueueEntity[id=${this.id}, type=${this.reportType}, status=${this.status}, operators=${this.operators.join(',')}]`;
    }

    public equals(other: QueuesEntity): boolean {
