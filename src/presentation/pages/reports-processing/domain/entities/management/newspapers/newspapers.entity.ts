...
export class NewspapersEntity implements Newspapers {
    ...
    public get reportDescription(): string {
        const reportDescription: Record<string, string> = {
            ABI: 'MANAGEMENT.NEWSPAPERS.OPTIONS.REPORT_TYPE.NO_INTERNET',
            CPS: 'MANAGEMENT.NEWSPAPERS.OPTIONS.REPORT_TYPE.PARTIAL_SIGNAL',
            CPO: 'MANAGEMENT.NEWSPAPERS.OPTIONS.REPORT_TYPE.PARTIAL_OPERATOR',
            ZOB: 'MANAGEMENT.NEWSPAPERS.OPTIONS.REPORT_TYPE.WHITE_ZONE',
        };
    ...
    public get sourceDescription(): string {
        const sourceDescription: Record<string, string> = {
            APP: 'MANAGEMENT.NEWSPAPERS.OPTIONS.SOURCE.APP',
            USSD: 'MANAGEMENT.NEWSPAPERS.OPTIONS.SOURCE.USSD',
            SMS: 'MANAGEMENT.NEWSPAPERS.OPTIONS.SOURCE.SMS',
            IVR: 'MANAGEMENT.NEWSPAPERS.OPTIONS.SOURCE.IVR',
        };
    ...
    public toString(): string {
        return `NewspapersNewspaperEntity[id=${this.id}, type=${this.reportType}, status=${this.status}, operators=${this.operators.join(',')}]`;
    }

    public equals(other: NewspapersEntity): boolean {
