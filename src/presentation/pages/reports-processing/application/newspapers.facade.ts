...
import { FetchNewspapersUseCase } from '../domain/use-cases/newspapers.use-case';
import { NewspapersFilter } from '../domain/value-objects/newspapers-filter.vo';

@Injectable({ providedIn: 'root' })
export class NewspapersFacade extends BaseFacade<
    NewspapersEntity,
    NewspapersFilter
> {
    readonly newspapers$: Observable<NewspapersEntity[]> = this.items$;
    ...
    constructor(
        private readonly fetchUseCase: FetchNewspapersUseCase,
    ...
    private shouldFetch(forceRefresh: boolean): boolean {
        ...
        if (isStale) {
            console.log('🕐 [NewspapersFacade] Data is stale, refetching');
            return true;
        }
