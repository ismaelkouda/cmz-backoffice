...
import { QUEUE_TABLE_CONST } from '@presentation/pages/reports-processing/domain/constants/queue-table.constant';
import { QueueEntity } from '@presentation/pages/reports-processing/domain/entities/queue/queue.entity';
...
export class TableQueueComponent {
    @Input() spinner!: boolean;
    @Input() listQueue$!: Observable<QueueEntity[]>;
    @Input() pagination$!: Observable<Paginate<QueueEntity>>;
    @Output() treatmentRequested = new EventEmitter<QueueEntity>();
    @Output() journalRequested = new EventEmitter<QueueEntity>();
    ...
    private readonly stateLabelMap: Record<string, string> = {
        accepted: 'REPORT_PROCESSING.QUEUE.OPTIONS.STATE.ACCEPTED',
        pending: 'REPORT_PROCESSING.QUEUE.OPTIONS.STATE.PENDING',
        rejected: 'REPORT_PROCESSING.QUEUE.OPTIONS.STATE.REJECTED',
    };
    ...
    public readonly table: TableConfig = QUEUE_TABLE_CONST;
    ...
    public onExportExcel(): void {
        this.listQueue$.pipe(take(1)).subscribe((queues) => {
    ...
    public getOperatorLabel(operator: string): string {
        ...
        switch (normalized) {
            case 'orange':
                translationKey =
                    'REPORT_PROCESSING.QUEUE.OPTIONS.OPERATOR.ORANGE';
                break;
            case 'mtn':
                translationKey = 'REPORT_PROCESSING.QUEUE.OPTIONS.OPERATOR.MTN';
                break;
            case 'moov':
                translationKey =
                    'REPORT_PROCESSING.QUEUE.OPTIONS.OPERATOR.MOOV';
                break;
            default:
                return operator;
        }
    ...
    public getReportTypeLabel(reportType: string): string {
        ...
        switch (normalized) {
            case 'couverture partielle signal':
                translationKey =
                    'REPORT_PROCESSING.QUEUE.OPTIONS.REPORT_TYPE.PARTIAL_SIGNAL';
                break;
            case 'zone blanche':
                translationKey =
                    'REPORT_PROCESSING.QUEUE.OPTIONS.REPORT_TYPE.WHITE_ZONE';
                break;
            case "absence d'internet":
                translationKey =
                    'REPORT_PROCESSING.QUEUE.OPTIONS.REPORT_TYPE.NO_INTERNET';
                break;
            default:
                return reportType;
        }
    ...
    public onActionClicked(item: QueueEntity): void {
        this.treatmentRequested.emit(item);
    }

    public getQueueTooltip(item: QueueEntity): string {
        const normalizedState = item.state?.toLowerCase() ?? '';
        if (normalizedState === 'pending' || normalizedState === 'rejected') {
            const queueLabel = this.translate.instant(
                'REPORT_PROCESSING.QUEUE.TABLE.QUEUE'
            );
            return `${queueLabel} ${item.uniqId}`;
        }
        return this.translate.instant('REPORT_PROCESSING.QUEUE.TABLE.SEE_MORE');
    }

    public onJournalClicked(item: QueueEntity): void {
