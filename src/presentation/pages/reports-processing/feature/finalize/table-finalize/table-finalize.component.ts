...
export class TableFinalizeComponent {
    ...
    private readonly stateLabelMap: Record<string, string> = {
        finalized: 'REPORT_PROCESSING.FINALIZE.OPTIONS.STATE.FINALIZED',
        pending: 'REPORT_PROCESSING.FINALIZE.OPTIONS.STATE.PENDING',
        rejected: 'REPORT_PROCESSING.FINALIZE.OPTIONS.STATE.REJECTED',
    };
    ...
    public getOperatorLabel(operator: string): string {
        ...
        switch (normalized) {
            case 'orange':
                translationKey =
                    'REPORT_PROCESSING.FINALIZE.OPTIONS.OPERATOR.ORANGE';
                break;
            case 'mtn':
                translationKey =
                    'REPORT_PROCESSING.FINALIZE.OPTIONS.OPERATOR.MTN';
                break;
            case 'moov':
                translationKey =
                    'REPORT_PROCESSING.FINALIZE.OPTIONS.OPERATOR.MOOV';
                break;
            default:
                return operator;
        }
    ...
    public getReportTypeLabel(reportType: string): string {
        ...
        switch (normalized) {
            case 'zone blanche':
                translationKey =
                    'REPORT_PROCESSING.FINALIZE.OPTIONS.REPORT_TYPE.WHITE_ZONE';
                break;
            default:
                return reportType;
        }
    ...
    public getFinalizeTooltip(uniqId: string): string {
        const finalizeLabel = this.translate.instant(
            'REPORT_PROCESSING.FINALIZE.TABLE.FINALIZE'
        );
