...
export class TableTreatmentComponent {
    ...
    private readonly stateLabelMap: Record<string, string> = {
        terminated: 'REPORT_PROCESSING.TREATMENT.OPTIONS.STATE.TERMINATED',
        'in-progress': 'REPORT_PROCESSING.TREATMENT.OPTIONS.STATE.IN_PROGRESS',
        rejected: 'REPORT_PROCESSING.TREATMENT.OPTIONS.STATE.REJECTED',
    };
    ...
    public getOperatorLabel(operator: string): string {
        ...
        switch (normalized) {
            case 'orange':
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.OPERATOR.ORANGE';
                break;
            case 'mtn':
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.OPERATOR.MTN';
                break;
            case 'moov':
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.OPERATOR.MOOV';
                break;
            default:
                return operator;
        }
    ...
    public getReportTypeLabel(reportType: string): string {
        ...
        switch (normalized) {
            case 'couverture partielle signal':
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.REPORT_TYPE.PARTIAL_SIGNAL';
                break;
            case 'zone blanche':
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.REPORT_TYPE.WHITE_ZONE';
                break;
            case "absence d'internet":
                translationKey =
                    'REPORT_PROCESSING.TREATMENT.OPTIONS.REPORT_TYPE.NO_INTERNET';
                break;
            default:
                return reportType;
        }
    ...
    public getTreatTooltip(uniqId: string): string {
        const treatLabel = this.translate.instant(
            'REPORT_PROCESSING.TREATMENT.TABLE.TREAT'
        );
