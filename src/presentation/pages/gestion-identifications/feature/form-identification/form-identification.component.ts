...
export class FormIdentificationComponent implements OnInit {
    ...
    async PostSupervisionOperationsDemandesServicesDetails(
        dataToSend = {
            tenant_code: this.simSelected?.['tenant_code'],
            transaction: this.simSelected?.['transaction'],
        }
    ): Promise<void> {
        const response: any = await handle(
            () =>
                this.gestionIdentificationsService.PostGestionIdentificationsDetails(
                    dataToSend
                ),
            this.toastrService,
    ...
    public getTitleForm() {
        switch (this.sim?.['operation']) {
            ...
            default:
                this.titleForm = this.sim?.['msisdn'];
        }
    }
    ...
    public initFormIdentification(): void {
        this.formIdentification = this.fb.group({
            ...
            type_personne: [
                { value: this.sim?.['type_personne'] ?? '', disabled: true },
            ],
            nom: [this.sim?.['nom'] ?? ''],
            ...
            accepte: [
                {
                    value: this.sim?.['accepte'] ?? '',
                    disabled: this.disabledPrendreField(),
                },
                Validators.required,
            ],
            commentaire: [
                {
                    value: this.sim?.['commentaire'],
                    disabled: this.disabledPrendreField(),
                },
            ],
        });
        if (this.typeTreatment.module === 'file-attente')
            this.formIdentification.disable();
        const typePersonneControl =
            this.formIdentification.get('type_personne');
    ...
    public isStartWorkedDay(): boolean {
        return (
            this.startedDay !== 'démarré' &&
            (this.userRole === 'superviseur' ||
                this.userRole === 'gestionnaire')
        );
    }

    async PostGestionIdentificationsFileAttentesTraiter(
        dataToSend = { ...this.formIdentification.value }
    ): Promise<void> {
