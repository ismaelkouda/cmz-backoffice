...
type ModalAction = { data: Folder, action: 'view-waiting-queue', view: 'modal' };
...
export class TableWaitingQueueComponent {
    ...
    constructor(private toastrService: ToastrService, private clipboardService: ClipboardService,
        ...
        private waitingQueueApiService: WaitingQueueApiService, private ngbModal: NgbModal) { }

    public onExportExcel(): void {
        this.listWaitingQueue$.subscribe(data => {
            if (data) { this.tableExportExcelFileService.exportAsExcelFile(data, this.table, "List_waitingQueue"); }
        });
    }

    public pageCallback() {
        this.waitingQueueApiService.fetchWaitingQueue({} as waitingQueueFilterInterface);
    }
    ...
    public getStepWaitingQueueBadge(selectedWaitingQueue?: { statut: T_BADGE_ETAPE; }): TYPE_COLOR_ETAPE_BADGE {
    ...
    public getStateWaitingQueueBadge(selectedWaitingQueue?: { statut?: T_BADGE_ETAPE; traitement?: T_BADGE_ETAT }): TYPE_COLOR_ETAT_BADGE {
        if (!selectedWaitingQueue || !selectedWaitingQueue.statut || !selectedWaitingQueue.traitement) {
            return 'badge-dark';
        }
        ...
        return stateMap[selectedWaitingQueue.statut]?.[selectedWaitingQueue.traitement] || 'badge-dark';
    }
    ...
    public handleAction(params: Action): void {
        ...
        switch (params.view) {
            case 'modal':
                if (params.action === 'view-waiting-queue') { this.handleWaitingQueueTreatment(params.data) }
                break;
        }
    }

    public handleWaitingQueueTreatment(selectedWaitingQueue: { statut: string, traitement: string }): void {
        this.visibleFormWaitingQueue = true;
        this.typeTreatment = this.getTreatmentButtonViewStyle(selectedWaitingQueue)?.typeTreatment;
    }
    ...
    private onSelectWaitingQueue(selectedWaitingQueue: Folder): void {
        ...
        this.waitingQueueApiService.setWaitingQueueSelected(selectedWaitingQueue);
    }
