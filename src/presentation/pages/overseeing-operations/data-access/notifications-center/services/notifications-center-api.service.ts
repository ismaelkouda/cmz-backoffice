...
} from '../interfaces/notifications-center.interface';

@Injectable()
export class NotificationsCenterApiService {
    private notificationsCenterSubject = new BehaviorSubject<
        Array<notificationsCenterInterface>
    >([]);
    private notificationsCenterPagination = new BehaviorSubject<
        Paginate<notificationsCenterInterface>
    >({} as Paginate<notificationsCenterInterface>);
    private notificationsCenterSelected =
        new BehaviorSubject<notificationsCenterInterface>(
            {} as notificationsCenterInterface
        );
    private loadingNotificationsCenterSubject = new BehaviorSubject<boolean>(
        false
    );
    private dataFilterNotificationsCenterSubject =
        new BehaviorSubject<notificationsCenterFilterInterface>(
            {} as notificationsCenterFilterInterface
        );
    private dataNbrPageNotificationsCenterSubject = new BehaviorSubject<string>(
        '1'
    );
    private apiResponseNotificationsCenterSubject =
        new BehaviorSubject<notificationsCenterApiResponseInterface>(
            {} as notificationsCenterApiResponseInterface
        );
    ...
    fetchNotificationsCenter(
        data: notificationsCenterFilterInterface,
        nbrPage: string = '1'
    ): void {
        if (this.loadingNotificationsCenterSubject.getValue()) return;
        this.loadingNotificationsCenterSubject.next(true);
        const url: string =
            notificationsCenterEndpointEnum.SUPERVISION_OPERATIONS_CENTRE_NOTIFICATIONS_ALL.replace(
                '{page}',
                nbrPage
            );
        ...
        this.http
            ...
            .pipe(
                ...
                finalize(() =>
                    this.loadingNotificationsCenterSubject.next(false)
                )
            )
            .subscribe();
    }
    ...
    getNotificationsCenterPagination(): Observable<
        Paginate<notificationsCenterInterface>
    > {
    ...
    setNotificationsCenterSelected(
        notificationsCenter: notificationsCenterInterface
    ): void {
